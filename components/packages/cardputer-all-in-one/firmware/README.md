...
（mainline `gud` 驱动，`/dev/dri/cardN`）+ UAC1（USB Audio Class 1，USB 音频类 1）半双工
麦克风/扬声器（mainline `snd-usb-audio`）+ USB 键盘（mainline `usbhid`，`/dev/input/eventN`），
同处一个 USB 复合设备。
...
native USB 归 TinyUSB 后，USB-C 上的串口控制台失效；GPIO43 又由扬声器 I2S WS 与麦克风 PDM CLK
共享，因此 firmware 默认设置 `CONFIG_ESP_CONSOLE_NONE=y`，不再启用 UART0 控制台。需要调试时使用
JTAG debugger，或在本地 sdkconfig 中把 UART TX 临时改到未占用 GPIO；不得恢复 GPIO43 上的 UART0 TX。
...
```bash
arecord -l                         # 应出现一个 USB Audio capture device
arecord -D hw:<card>,0 --dump-hw-params -f S16_LE -c 1 -r 16000 -d 10 /tmp/cardputer.wav
aplay /tmp/cardputer.wav           # 回放刚录制的 10 秒音频
```

两个方向的 `--dump-hw-params` 均应报告 `FORMAT: S16_LE`、`CHANNELS: 1`、`RATE: 16000`。先单独验证
播放和录音，再在保持一个 PCM 打开的情况下启动另一个，确认最后启动方向接管、关闭后原方向恢复。
连续运行 10 分钟，确认无周期性爆音、停声或 USB 重新枚举；同时重复 `modetest -M gud` 和 `evtest`
做复合回归。
...
| `main/usb_descriptors.{c,h}` | USB 复合描述符数据（GUD + UAC1 playback/capture + HID）+ HID 回调 |
...
| `main/uac_audio.{c,h}` | UAC1 控制回调、半双工仲裁、I2S 扬声器与 PDM 麦克风数据链路 |
