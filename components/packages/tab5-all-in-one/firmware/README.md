...
| **ISP 的 WBG（白平衡增益）、BLC、crop** | ❌ 不可用（均要 rev ≥3.0） | ⇒ **白平衡只能改用 CCM**（色彩校正矩阵）来做，见下文「UVC 摄像头 / 白平衡」。**BLC 另有一条替代路**：SC202CS 传感器自带 BLC（寄存器 `0x3902`，`0xc0` = 开），由 `CAM_SENSOR_BLC_ENABLE` 控制，**默认 0 = 只读不写**。官方标定的 `acc.blc = 16/255` 在本板没有任何 ISP 侧执行点 ⇒ 黑电平只能靠传感器自己处理。⏳ **基座实测值尚未取得**（测法见「上板验证清单」阶段 4） |
| **ISP 的 CCM** | ✅ 可用，但**系数上限 4.0** | rev <3.0 的 CCM 系数是 **2 位整数 + 10 位小数**；rev ≥3.0 才是 4 位整数 + 8 位小数（上限 16）。控制律必须把增益钳在 4.0 以内。⚠️ **不要贴着 4.0 写**：驱动的范围检查是**闭区间** `[-4.0, +4.0]`（`isp_ccm.c:25-31`），而 S2.10 实际能表达的最大值是 `4095/1024 = 3.9990` ⇒ 恰好写 4.0 会**通过**驱动检查，然后在 HAL 里被 `saturation = true` **悄悄截断且不报错** —— 硬件里的矩阵与日志打出来的不是同一个。本工程的钳制阈值取 **3990 milli**（`CAM_CCM_ABS_MAX_MILLI`） |
...
### 状态：出真实画面 + AE 闭环已实机验证；**整轮 ISP 对齐（10 个 commit）全部未上板**
...
| **AE（自动曝光）闭环**（软件全帧均值 + 目标 120 的那一版） | ✅ 实机 |

**以下全部是「ISP 对齐」那一轮（`751b24bb` … `e6201fc3` 共 10 个 commit）引入的，
⛔ 一次都没有烧过板**。它们只经过宿主机验证（9 组共 3258 个用例 + 两档零告警构建 +
`check_usb_desc.py` 两档 + `isp_cal_extract.py --check`）。
逐项的上板测法、判据、失败回退见下文「**上板验证清单**」。

| 项 | 开关 | 状态 |
|---|---|---|
| ISP gamma（官方 4 档曲线）+ 统计侧逆 gamma 保线性域 | `CAM_GAMMA_ENABLE` | ⏳ **未上板验证** |
| AE 目标 120 → 官方 62、死区改成非对称 [56, 64] | `CAM_AE_TARGET` 等 | ⏳ **未上板验证** |
| BF 去噪 + Demosaic 梯度（按增益查官方表） | `CAM_ADN_ENABLE` | ⏳ **未上板验证** |
| LSC 暗角修正（273 格 × 4 通道） | `CAM_LSC_ENABLE` | ⏳ **未上板验证** |
| SHARP 锐化 + Color 对比度（按增益查官方表） | `CAM_AEN_ENABLE` | ⏳ **未上板验证** |
| 硬件 AE 5×5 统计（官方加权测光 + quorum 剔除） | `CAM_AE_STAT_ENABLE` | ⏳ **未上板验证** |
| AE 反馈量切到硬件 5×5 | `CAM_AE_SOURCE` | ⏳ **未上板验证** |
| 黑电平量具（线性域逐通道最暗值）+ 传感器 BLC 开关 | `CAM_SENSOR_BLC_ENABLE`（默认 **0**） | ⏳ **未上板验证**，**基座实测值未取得** |
| 硬件 AWB 白点统计（官方白点框，采在 CCM 前） | `CAM_AWB_STAT_ENABLE` | ⏳ **未上板验证** |
| AWB 估计器切到硬件白点、**绝对**增益公式 | `CAM_AWB_SOURCE` | ⏳ **未上板验证（风险最高的一项）** |
| CCM 换成官方 19 档按 CCT 插值 + WB 右乘折叠 + 强度钳制 | `CAM_CCM_MODE` / `CAM_CCM_STRENGTH_MAX` | ⏳ **未上板验证**，强度默认 192 是**降级档**（等黑电平实测） |
| 直方图统计 + `env.luma` 重建 | `CAM_HIST_ENABLE` / `CAM_ENV_MODEL` | ⏳ **未上板验证**，重建式是 **[反推]** 不是确证 |
| gamma 按 `env.luma` 动态选档 + 背光切 AE 目标 | `CAM_GAMMA_ADAPTIVE` / `CAM_BACKLIGHT_ENABLE` | ⏳ **未上板验证** |
| LSC 按 CCT 选档 + 饱和度按 CCT 两档 | `CAM_LSC_BY_CT` / `CAM_SAT_BY_CT` | ⏳ **未上板验证** |
| 灰世界四道防护的实际效果（`CAM_AWB_SOURCE = 0` 回退路径） | `CAM_AWB_ENABLE` | ⏳ **未上板验证** |
| 五项能力同跑 10 分钟的复合回归 | — | ⏳ **未做**（规程见下文「五项能力复合回归规程」） |
| 摄像头开着时 GUD 帧率的变化、PSRAM 带宽争用实测值 | — | ⏳ **未测**（观测设施已就位；采集时机就在复合回归里） |
| 脏矩形 / LZ4 的定量验证（P0 欠账） | — | ⏳ **未测**（同上，复合回归顺带采集） |
...
> 这一场还没跑。
>
> ⚠️ **全部 ISP 对齐开关翻到「关」= 回到 `cb8aa3f5`（T4）之前的行为**，
> 但那也不是已验证状态 —— **唯一实机验证过的画质配置是 `43288ae9`（T0 之前）**：
> 对角阵 CCM `diag(1.7, 1, 1.55)` + 软件全帧均值 AE（目标 120）+ 灰世界 AWB，
> **无 gamma、无 LSC、无 BF/SHARP/Color、无任何硬件统计块**。
> 完整的「关到底 = 退到哪」对照见「上板验证清单」末尾那张表。
...
**这正是官方在 rev <3.0 上的降级路径**，不是我们发明的绕法 ——
`esp_video_isp_device.c` 的 `isp_init_ccm_param()` 同样把 R/B 增益乘进 CCM 的第 0/2 列。

#### 现在的形态：官方 19 档矩阵 + 白平衡右乘折叠（`CAM_CCM_MODE = 1`）⏳ 未上板验证

对角阵只做白平衡、不做色彩还原。ISP 对齐那一轮把它换成了**官方标定的 3×3 矩阵**：
按 CCT 在 19 档（1200 K … 12000 K）之间逐元素线性插值取基础矩阵 `M`，
再把白平衡增益**右乘**折进去，最后按 S2.10 钳制：

```
P(t) = ((1 − t)·I + t·M) · diag(kr, 1, kb)        t ∈ [0, 1]，以 1/256 计
```

三条必须说清的：

1. **为什么右乘是对的（而不是「官方按 WBG 前提标的、不能这么用」）** ——
   官方每档矩阵的**行和都是 1.000**。右乘 `diag(kr,1,kb)` 只缩放列、**不改行和**；
   中性面 `w = (1/kr, 1, 1/kb)` 经 `M·W` 后精确映射到 `(1,1,1)` ⇒ 白还是白、整体增益仍为 1。
   WBG 与 demosaic 都是逐通道线性算子、可交换 ⇒ 与「先 WBG 再 CCM」**数学等价**。
   宿主机用例把「行和恒为 1000（截断误差 ≤ 2）」与「中性面三分量相对差 ≤ 1%」钉死了。
2. **钳制用「朝单位阵混合」而不是整体缩放** —— `M(t)` 的行和恒为 1 ⇒ 不损失亮度；
   整体缩放 `αM` 要靠 AE 补 `1/α` 曝光，2292 K 那档要多花 **2 EV** 的噪声。
   且 **`t = 0` 恰好退化成对角阵** —— 钳到底的最坏情况就是回到今天这条已实机验证的路。
   可行域是 `[0, t*]`（凸性保证），`t*` 由 8 次整数二分求出。
3. **强度总闸 `CAM_CCM_STRENGTH_MAX` 默认 192 是降级档，不是终值** ——
   它的判定点是**黑电平实测**（见「上板验证清单」阶段 4）：官方 `acc.blc = 16` 在本板
   无 ISP 执行点，而一个**等值**基座 `p` 被 `diag(kr,1,kb)` 变成**不等值**基座后，
   会被官方 CCM 的大负非对角项放大成可见的**品红黑位**
   （5040 K + kr 1.8 / kb 1.85 + p = 16 ⇒ 约 `(+23, −8, +21)`）。
   拿到实测基座后按 `cam_tune.h` 里那张表放开到 256，或保持 192 并把
   `CAM_STAT_BLC_PEDESTAL` 一起改成 16。

**一行回退**：`CAM_CCM_MODE = 0` ⇒ 逐元素退回 `diag(kr, 1, kb)`，即 T9 之前的形态。
...
| 闸 | 宏 | 值 | 为什么 |
|---|---|---|---|
| ① 死区 | `CAM_AE_DEADBAND` | **6**（`CAM_AE_SOURCE = 1` 时直接用官方非对称区间 **[56, 64]**） | ≈ 目标的 10%，肉眼看不出，却足以让量化噪声与一片云飘过就触发调整（= 画面「一亮一暗地喘」）。⚠️ 目标从 120 改成官方的 62 时**必须跟着缩**：留在 12 会让相对死区从 10% 变成 19%，AE 会在 50~74 任何一处停下。⚠️ 官方死区**不对称**（−6/+2）是有物理含义的：过曝削顶丢掉的信息任何后级都补不回来，偏暗只是噪声大一点、还能靠 gamma 提回来 |
...
#### 目标为什么从 120 改成 62 ⏳ 未上板验证

120 是我们拍脑袋定的，而这颗传感器在实测那间屋子里线性均值只到 45 ⇒ **AE 永远够不着**
⇒ 曝光顶格、增益推满，换回来的仍是暗图，只是噪声按满增益的量级 ——
用户看到的「噪声重」正是这个。62 = 官方 `agc.luma_adjust.target`。

#### 反馈量的来源：`CAM_AE_SOURCE` ⏳ 未上板验证

| 值 | 反馈量 | 采样点 |
|---|---|---|
| 0 | 软件全帧 1/64 抽样的**线性**亮度（过逆 gamma 表还原） | ISP **输出**（CCM / gamma / Color 的**下游**） |
| **1（默认）** | ISP 硬件 **5×5 分块**的官方加权均值 + 过暗/过亮块 quorum 剔除 | **demosaic 之后**（CCM 的**上游**） |

换源的理由：官方那套 `target 62` / 死区 `[56, 64]` / 阈值 `14·239` / quorum `5·3` /
25 个权重**全部**是在 `AFTER_DEMOSAIC` 上标定的 —— 不换源，那些数只是一串抄来的数字。
且硬件采样点在 CCM 上游 ⇒ AWB 改 CCM、gamma 换档都扰不到 AE 的输入，两个环真正解耦。

⚠️ **换源之后画面会变亮，这不是回归。** 本板没有 WBG、白平衡是拿 CCM 对角线做的
⇒ 旧的软件统计采在白平衡**下游**，新的硬件统计采在**上游**：

```
软件线性 ≈ 硬件加权 × (0.30·kr + 0.586·kg + 0.113·kb) = ×1.271     ⇒ ρ ≈ 0.79
```

也就是说旧口径下写 62，实际是把 WB 放大过的亮度顶到 62、等价于把真实亮度压到 49，
**一直在系统性欠曝 27%**。这个 **ρ ≈ 0.79 是整段论证的验算**，自检行实时打着 ——
**实测显著偏离 0.79 就说明「硬件采样点在 CCM 上游」这条推导错了**，
那会推翻 `CAM_AE_SOURCE = 1` 的全部依据（见「上板验证清单」阶段 3 的判定点 ②）。

⚠️ 计划原文要求「换源必须按 ρ 缩放目标以保持观感不变」（120 × 0.79 = 95），
**本实现有意不这么做**：那套算术的前提是「旧目标没有权威含义」，而目标改成官方的 62
之后前提就没了 —— **62 本来就是新口径下的权威值，再乘一次 ρ 等于把它搬回错误的域。**

#### 目标随场景走：背光 → 暗部优先 ⏳ 未上板验证

官方 `agc.{high,low}_light_priority.luma_offset` 是 **−3 / +1** ⇒ 实际目标
**59**（高光优先，官方对这颗传感器的默认模式）或 **63**（暗部优先）。
官方的模式选择器闭源，这里给的是可解释版本：**亮块占比 ≥ 25% 且场景均值 < 56**
（两个量都来自同一次直方图采样，与 `env.luma` 同源）。
25% 与 `target_low` 是**我们定的**，所以放在 `cam_tune.h` 而不是 `cam_isp_cal.h`。
一行回退：`CAM_BACKLIGHT_ENABLE = 0` ⇒ 恒取高光优先。
...
### 自动白平衡（AWB）：硬件白点统计（默认）/ 灰世界 + 四道防护（回退）⏳ 全部未上板

> ⚠️ **本节描述的两条路都没有实机验证过。**
> 它们只过了宿主机用例（`test_cam_tune.c` 521 条 + `test_cam_isp_map.c` 532 条，
> 含红墙/绿植/蓝天三个单色场景、带 3 拍滞后的 AE+AWB 联合仿真、以及硬件路的 200 拍不动点）。
> **`CAM_AWB_ENABLE = 0` 一行即可退回静态白平衡**（相关代码整段不编译）。

**两条路，一个开关切**（`CAM_AWB_SOURCE`）：

| 源 | 估计器 | 采样点 | 建议值公式 | 环路性质 |
|---|---|---|---|---|
| 0（回退） | 软件灰世界，整幅均值 | ISP **输出**（CCM **下游**，再逆 gamma 回线性） | `sug = cur × g_mean / chan_mean` | **闭环**（改增益会改统计） |
| **1（默认）** | 硬件白点筛选，官方框逐**像素**筛 | **CCM 之前**（`BEFORE_CCM`，官方 esp_video 的硬编码采样点） | `sug = Σg / Σchan` | **开环前馈**（改增益不影响统计） |

⚠️⚠️ **这两条公式绝不能串味。** 把源 0 的公式用在源 1 上，每一拍都会把已经生效的增益
再乘一遍，增益单调跑到限位；而现场表现是「颜色缓慢地越来越偏，可所有计数器都显示
一切正常」—— 最难归因的那一类。为此做了**三重防呆**，缺一不可：

1. **签名里根本没有 `cur`** —— `cam_awb_step_hw(st, s, ae_converged)` 拿不到当前增益，
   **结构上写不出**乘 `cur` 的代码，不是靠注释提醒；
2. **旧函数原样保留** —— `cam_awb_step()` 一行不动，留给源 0。两条路各一个函数，
   **不做「加个参数复用一份实现」那种合并** —— 那正是两个公式串味的入口；
3. **宿主机不动点用例** —— 固定统计连喂 200 拍。⚠️ 这里有个陷阱：错误实现**同样会停住**
   （顶到 3445 之后返回原值 ⇒ `SKIP_QUANT`），所以「最后 50 拍 Δ == 0」单独一条
   **抓不住它**。真正的判据是另外三条：停下来的**值** = `1/rg`、`1/bg` 且严格小于上限；
   停下来的**理由**是「死区内」且 `SKIP_QUANT` 恒为 0；从**高于**不动点的初值
   （上限 3445）出发必须**往下**走 —— 错误实现只会一直贴着上限。

源 1 上判据全部换成官方标定值：「色偏过大」与「暗场/过亮」**取消**（硬件的白点框与
`white_patch.luminance` 窗已经在像素级做了同一件事，这三个计数器恒为 0），
新增「**白点太少**」（`counted < 1200` = 官方 `awb.min_counted` ⇒ 整拍不动）与
**重心判据**（`rg/bg` 必须落在官方盒子 `[3801,8790] × [2903,6587]` 内）。
增益上限 3000 → **3445** = `10⁷/2903`（官方白点轨迹端点给出的 kb 最大值，2289 K 白炽灯）
—— 留在 3000 会在暖光下顶住，表现为「白炽灯下永远偏黄」。
**盒子与限幅自洽**：盒子推出的 `kr∈[1138,2631]`、`kb∈[1518,3445]` 整个装进 `[1000,3445]`
⇒ 两道判据不会互相打架，由 `_Static_assert` 与宿主机用例各钉一遍。

⚠️ **周期由谁分频**（与计划不符的一处，已修正）：计划让 `cam_awb_step_hw()` 自己分频，
而调用点又只在触发 oneshot 的那一拍调它 —— **两处都分频，实际周期会变成
`INTERVAL² = 100` 拍 = 10 秒一次**，现场表现是「AWB 反应特别迟钝」。
本实现按「每拿到一份新采样调一次」，周期只由 oneshot 的触发节奏决定。
副作用：源 1 上 `reasons[]` 直方图按**秒**累加（源 0 按帧），`SKIP_PERIOD` 恒为 0。

以下是**源 0（回退路径）**的结构，保留不动：
...
| 防护 | 宏 | 值 | 挡什么 |
|---|---|---|---|
| ① 亮度下限 | `CAM_AWB_LUM_MIN` | **20** | 太暗时通道均值由**读出噪声**主导（信噪比 <1），比值纯属随机。⚠️ AE 目标从 120 改成 62 时**必须跟着缩**（≈ 目标的三分之一）：留在 40 的话 40 就成了新目标的 65%，正常曝光的画面会被判成「暗场」⇒ AWB 一步都不走 |
...
#### 为什么从「不挂硬件 AWB 统计」改成「挂」——**原论证自己写明的失效条件被触发了**

这里原本有一整节论证「不挂 `isp_awb`」，落脚点是第 2 条：

> **它唯一能多给的那份能力在本板上恰好残缺** —— 硬件 AWB 的价值在**白点筛选**
> （只累加落在指定亮度/色度框内的像素），但**那个框本身要靠色卡标定**
> （把猜测从「灰世界成不成立」挪到「白点框画得对不对」）。

**这个前提在 T0 之后不成立了**：官方 `sc202cs_default.json` 的 `awb.range` 已经机械提取进
`cam_isp_cal.h`，且它与我们的采样点**同域** —— 官方是在**未做 WB 的 raw 色度空间**里标的
（`rg ∈ [0.38, 0.88]`），而本板 rev v1.0 同样没有 WBG、同样采在 CCM 之前。
框不再是猜测，那条论证的落脚点就没了。
...
| 原理由 | 现状 |
|---|---|
| ① 反馈量已经免费拿到、硬件换不来新信息量 | **不成立了**：白点筛选是**像素级**的，灰世界是整幅平均 —— 镜头怼着彩色物体时前者仍然可信，这正是换估计器的收益 |
| ② 白点框要色卡标定 | **失效条件被触发**（见上） |
| ③ subwindow 在 rev <3.0 被砍 | **仍然成立** ⇒ 所以**只用主窗、不配 subwindow**。而且更进一步：`esp_isp_awb_isr()` **无条件**读 25 个 subwindow 的 LUT（100 次寄存器往返 + 416 字节 ISR 内拷贝），本板上全是空的 ⇒ **只用 oneshot、绝不开连续模式**（连续 = 每帧一次这样的 ISR，oneshot = 每秒一次） |
| ④ 不新增组件目录 / 不新增运行期失败路径 | 组件目录仍是 **12 个**（`isp_awb` 是 IDF 内置的 `esp_driver_isp`，不算依赖）；失败路径确实多了一条，处置是**只降级不拦启动** —— 统计块建不起来时记下错误码、自检行照打，AWB 继续吃软件灰世界 |

⚠️ **`esp_isp_awb_controller_get_oneshot_statistics()` 的 `timeout_ms` 不能传 0**。
驱动会把 0 解读成「触发后立刻返回、结果走回调」，而我们没注册回调 ⇒ 它会跳过等待、
立刻 `isp_ll_awb_enable(false)`，统计根本来不及完成。
现场表现是「**触发在涨、白点恒 0**」。本工程取 `CAM_AWB_ONESHOT_MS = 60`。
（AE 的 oneshot 文档说 `timeout_ms = 0` 可以在回调里拿结果，**AWB 这条路走不通**。）
...
### ISP 对齐：与官方管线的逐级对照 ⏳ **整章未上板验证**

> **口径**：官方 = Espressif `esp_video` + `esp_ipa` + 标定文件
> `sc202cs_default.json`（随 `esp_ipa` 组件分发，Apache-2.0）。
> 逻辑重建见 `docs/superpowers/research/2026-08-19-esp32p4-official-isp-pipeline.md`，
> 对齐前基线见 `…-our-isp-pipeline-audit.md`，实施计划见
> `docs/superpowers/plans/2026-08-21-tab5-isp-align-with-official.md`。
>
> ⛔ **本章描述的全部配置都没有烧过板。** 每一项的上板测法与失败回退见「上板验证清单」。

#### 逐级对照表：官方 13 级 vs 我们配了哪些

| # | 级 | 域 | rev v1.0 | 官方配不配 | **我们** | 参数来源 |
|---|---|---|---|---|---|---|
| 1 | **BLC** 黑电平 | RAW | ❌ 不可用 | eco5 配 / eco4 不配 | ❌ 改用**传感器自带 BLC**（`0x3902`，默认**关**） | — |
| 2 | **DPC** 坏点 | RAW | 寄存器有、**IDF 无 API** | 不配 | ❌ 不配 | — |
| 3 | **BF** 双边降噪 | RAW | ✅ | 配 | ✅ `CAM_ADN_ENABLE`，按增益 **7 档** | `adn.bf`（level + 3×3 模板） |
| 4 | **LSC** 暗角 | RAW | ✅（门是 rev ≥ **1.0**） | 配（9 档色温） | ✅ `CAM_LSC_ENABLE`，**273 格 × 4 通道**，按 CCT **3 档最近邻** | `acc.lsc`（9 档取 3 档） |
| 5 | **Demosaic** | RAW→RGB | ✅ | 配 | ✅ 只 `configure` 不 `enable`（已被 `output=RGB565` 隐式打开），按增益 **4 档** | `adn.demosaic.gradient_ratio` |
| 6 | **Median** | RGB | 寄存器有、**无 API** | 不配 | ❌ 不配 | — |
| 7 | **CCM** | RGB 线性 | ✅ S2.10（±4.0） | 配（19 档色温） | ✅ `CAM_CCM_MODE`，**19 档按 CCT 线性插值** + WB 右乘折叠 + 强度钳制 | `acc.ccm.table` |
| 8 | **Gamma** | RGB | ✅ | 配（R/G/B 各 16 点折线） | ✅ `CAM_GAMMA_ENABLE`，**4 档**按 `env.luma` 动态选 | `aen.gamma`（γ 解析生成 y[16]） |
| 9 | **RGB2YUV** | — | ✅（复位即开） | 隐式 | ✅ 由输出格式隐式决定 | — |
| 10 | **SHARP** 锐化 | YUV(Y) | ✅ | 配（按增益 4 档） | ✅ `CAM_AEN_ENABLE`，按增益 **4 档** | `aen.sharpen` |
| 11 | **Color** 对比度/饱和度 | YUV | ✅ | 配 | ✅ `CAM_AEN_ENABLE`：对比度按**增益** 4 档、饱和度按 **CCT** 2 档；色调/亮度写 0 | `aen.contrast` / `acc.saturation` |
| 12 | **YUV2RGB / YUV Limit** | — | ✅ | 隐式 | ✅ 由输出格式隐式决定 | — |
| 13 | **CROP** | 输出域 | ❌ 不可用 | 不配 | ❌ 不配（PPA 顶替） | — |
| — | **WBG** 白平衡增益 | RAW | ❌ 不可用 | 不配（rev<3.0） | ❌ **折进 CCM 第 0/2 列**（官方 rev<3.0 同款降级） | — |

统计块（都是旁路取样、不改数据）：

| 统计块 | 采样点 | 我们 | 频率 | 用途 |
|---|---|---|---|---|
| **AE** 5×5 | `AFTER_DEMOSAIC`（CCM **上游**） | ✅ `CAM_AE_STAT_ENABLE`，**连续**模式 | 每帧 | AE 反馈量（`CAM_AE_SOURCE = 1`） |
| **AWB** 白点 | `BEFORE_CCM` | ✅ `CAM_AWB_STAT_ENABLE`，**oneshot** | 1 秒（`CAM_AWB_INTERVAL_TICKS = 10`） | AWB 反馈量 + **CCT 估计**（喂 CCM / LSC / 饱和度三个消费者） |
| **HIST** 16 段 | RGB（demosaic 后） | ✅ `CAM_HIST_ENABLE`，**oneshot** | 1 秒（相位 **错开半周期** `CAM_HIST_PHASE = 5`） | `env.luma` 重建 → gamma 选档；亮块占比 → 背光判定 |
| **AF** | RGB2YUV 后（固定） | ❌ 不建 | — | SC202CS 定焦，无对焦马达 |
| **AWB subwindow** 5×5 | — | ❌ **rev <3.0 硬件没有** | — | 官方的分区投票在本板做不了 |
...
```
                     ┌───────────── AE 环（快，300 ms）─────────────┐
                     │                                              │
 SC202CS ──RAW──▶ BF ─▶ LSC ─▶ Demosaic ─┬─▶ CCM ─▶ Gamma ─▶ SHARP ─▶ Color ─▶ 输出
   ▲  曝光/增益                          │     ▲                              │
   │                          [AE 5×5]───┘     │                   [软件抽样]─┘
   └──────────────────────────────────────┐    │                        (源=0 回退)
                                          │  [AWB 白点]
                          [HIST]──env.luma┘    │
                             │                 └──── AWB 环（慢，1 秒）→ kr/kb → CCM
                             ▼
                        gamma 选档 / 背光判定
```
...
1. **两个统计抽头都在被控块的上游** —— AE 采在 demosaic 后（CCM 上游），
   AWB 采在 CCM 前。⇒ AWB 改 CCM、gamma 换档、Color 换档都**扰不到**两个环的输入，
   **两个环真正解耦**，不需要靠时间尺度分离来凑合。
   （代价：AWB 因此从闭环变成**开环前馈**，建议值公式必须跟着改，见上文三重防呆。）
2. **时间尺度仍然分离** —— AE 300 ms / AWB 1 秒 / HIST 1 秒（相位错开）。
   这条现在是为了**估计噪声的时域平滑**，不再是环路稳定性。
3. **前馈级一律不带反馈** —— BF / Demosaic / SHARP / Color 的对比度按**增益**查表，
   LSC / 饱和度按 **CCT** 查表，都是开环查表 + 迟滞，官方 `adn` / `aen` 模块本身就是这么做的。
   ⚠️ 「对比度按 gain 选档、gain 由 AE 定」确实构成一条 AE→gain→对比度→亮度→AE 的回路，
   但四档之间只差 1.5%、换档还有 3 拍迟滞，**环增益远小于 1，不会自激**。

#### rev v1.0 的硬件限制清单（本章一切取舍的来源）

| 限制 | 后果 | 我们的处置 |
|---|---|---|
| **无 WBG** | 白平衡没有 RAW 域执行点 | 折进 CCM 第 0/2 列（官方同款降级） |
| **无 ISP BLC** | 官方 `acc.blc = 16` 无执行点 | 传感器自带 BLC（默认关）+ 统计侧减法 + CCM 强度钳制，**三级处置** |
| **CCM 是 S2.10（±3.9990）** | 官方 2292 K 那档本身含 4.5445，**装不下** | 朝单位阵混合的强度参数 `t`，8 次二分求最大可行 `t` |
| **无 crop** | ISP 出不了 640×360 | PPA SRM ×0.5 顶替（本来就要用它做旋转） |
| **无 AWB subwindow** | 官方 5×5 分区投票做不了 | 只用主窗的 4 个累加值 |
| **无影子寄存器** | 参数写下去**立刻生效**，没有帧边界原子性 | **所有换档一律带迟滞**（`CAM_FEEDFWD_HYST_TICKS = 3` 拍；LSC 因为要写 273×2 条 LUT，用**两倍** = 6 拍）。⚠️ 换光源时仍可能闪一下暗角 —— **这是硬件限制，不是 bug** |
| **CSI 桥无颜色转换** | `input != output` 直接 `ESP_ERR_NOT_SUPPORTED` | CSI 两端都填 RGB565，去马赛克全交给 ISP |

#### 标定数据从哪来：`test/isp_cal_extract.py`

`main/cam_isp_cal.h` 是**机械生成物，不许手改**。来源是 `esp_ipa` 组件里的
`sc202cs_default.json`（Apache-2.0，随 `idf.py build` 拉进 `managed_components/`）。

```bash
. $HOME/esp/esp-idf/export.sh
cd firmware
idf.py build                          # 先把 managed_components/ 拉下来
python3 test/isp_cal_extract.py --check   # 校验：生成物与 JSON 一致（md5 + 字节数双指纹）
python3 test/isp_cal_extract.py           # 重新生成（换传感器/换 IDF 版本时）
```

- **JSON 不在仓库里**（`managed_components/` 被 gitignore）⇒ 复现的第一步是 `idf.py build`；
- `--check` 是**烧板前的闸门之一**：改一个数字就失败；
- 234 KB 的 JSON 只提取 **7.2 KB**（LSC 9 档只带 3 档是最大的一笔），未提取的段在脚本里逐条写了理由；
- 脚本自带断言：LSC 增益值域必须落在硬件 `isp_lsc_gain_t`（2 整数位 + 8 小数位，上限 3.996）内
  —— 实测 **0.993 … 3.323（R 通道四角）⇒ 定点 851 / 1023，余量只有 17%**，
  两份基线文档都没提这个数。
...
| 项 | 官方 | 我们 | 原因 |
|---|---|---|---|
| BLC | ISP BLC，`acc.blc = 16` | 传感器自带 BLC（默认**关**） | rev v1.0 的 ISP BLC 不可用 |
| WBG | 独立 RAW 域增益块 | 折进 CCM 第 0/2 列 | rev v1.0 无 WBG（官方 rev<3.0 也这么降级） |
| CCM 强度 | 全量 | 低/高色温端按 `t` 钳制 | S2.10 上限 3.9990；2292 K 档本身含 4.5445 |
| AWB 子窗 | 5×5 分区投票 | 只有主窗 4 个累加值 | rev v1.0 无 subwindow |
| AWB 统计频率 | 每帧（连续） | 每秒（oneshot） | rev v1.0 的 AWB ISR **无条件**读 25 个空子窗（100 次寄存器往返 + 416 B ISR 内拷贝） |
| LSC 色温档 | 9 档 | **3 档**（2410 / 5210 / 8200 K） | 通道间差异只有 R 显著（`max\|2410K − 8200K\|`：r 0.711 / gr 0.175 / gb 0.203 / b 0.176），3 档换 13 KB flash 与换档开销 |
| LSC 档间 | [缺口] | **最近邻，不插值** | 273×4 个值插值要 1092 次乘加，而三档之间最大差 0.36，在 ~3× 的四角提亮上看不出来 |
...
1. **CCM 行和不变式是一切的地基。** 官方每档矩阵行和 = 1.000。任何对 CCM 的加工
   （右乘 WB、强度混合）都必须**保住行和**，否则中性面出来就不中性、整体增益也不再是 1。
   「整体缩放 `αM`」正是因为破了这条才被否掉。宿主机用例把行和与中性面各钉一条。
2. **CSI 的颜色转换在 rev <3.0 上不存在。** `input != output` ⇒ 连控制器都建不出来。
   IDF 例程 `mipi_isp_dsi` 那份 `RAW8→RGB565` 的 CSI 配置**只适用于 rev ≥3.0**。
3. **逆 gamma 表必须与硬件曲线同源。** 被逆的是**硬件那条 16 点折线**，不是解析式 γ
   —— 两者在首段差到十几级（γ=0.5 档 `x=8` 处解析式 45、折线 32）。
   所以「配硬件」与「建逆表」绑在**同一个函数**（`camera_gamma_apply()`）里，
   不给它们各自走一条路的机会。用错邻档的误差是十几级量级，宿主机用例专门钉住。
4. **AWB 的建议值公式随采样点改变。** 采在 CCM 后 = 闭环 ⇒ 必须乘 `cur`；
   采在 CCM 前 = 开环 ⇒ **绝不能乘 `cur`**。误用的现场表现是「颜色缓慢地越来越偏，
   而所有计数器都显示正常」。三重防呆见上文。
5. **直方图的 25 个权重之和必须精确等于 256。** 驱动 `s_esp_isp_hist_config_hardware()`
   硬性检查。⚠️ **基线文档记的那组「10 为主 / 内十字 11 / 中心 12」加起来是 260，
   照抄会直接 `ESP_ERR_INVALID_ARG`。** 本工程取 IDF 测试那组 `24×10 + 中心 16 = 256`。
   RGB 三个系数的 `integer` 域必须为 0；小数部取 **86/85/85** 让和恰好 256
   （官方用 85/85/85 = 255，驱动不检查系数和，两者都能配上）。
   15 个阈值必须严格落在 `(0, 256)` ⇒ 取 16 的整数倍。
6. **统计窗口不写就是 `bsize = 0`。** 现场表现是「25 块全 0」/「16 个 bin 全 0」。
   AE 的窗按 `/5` 分块（写 1280×720，整除）；**AWB 的窗是把四个坐标原样写进
   `lpoint`/`rpoint` 的闭区间 ⇒ 要写 1279/719**，两者口径不同。
7. **`esp_isp_bf_configure(proc, NULL)` 是空指针解引用**（`else` 分支之后仍无条件求值
   `config->flags.update_once_configured`）。`sharpen` / `color` 同样的写法。
   **全程不许传 NULL**，要禁用请用编译开关。
8. **Demosaic 只 `configure`、不 `enable`** —— 它已被 `output = RGB565` 隐式打开，
   调 `enable` 会撞 FSM 门拿到 `INVALID_STATE`，让人误以为参数没配上。
9. **`intr_priority` 与处理器不一致时驱动返回的是布尔 `1`，不是 `esp_err_t`**
   （`ESP_GOTO_ON_ERROR(intr_priority != isp_proc->intr_priority, …)` 传的是比较结果）。
   自检行会打出一个 `esp_err_to_name()` 认不出的码，**别往别处查**。三个统计块共用一个
   ISP 中断，全部写 0。
10. **`esp_isp_awb_controller_get_oneshot_statistics()` 不能传 `timeout_ms = 0`**（见上文）。
11. **LSC 的分配顺序**：`allocate_gain_array()` 要求 `lsc_fsm == INIT` ⇒ 必须在 `enable` 之前；
    而 `configure()` 没有 FSM 门 ⇒ 取流中可重配（按 CCT 换档依赖这一点）。
12. **定点换算要「进位」不能「截断」。** `cam_map_to_fixed()` 做四舍五入 + 进位 +
    溢出显式返回 false。不进位的话舍上去的小数会被位域**静默截断成 0**（3.99 变 3.00），
    而现场只看得到「锐化偏弱」这种说不清的偏差。
13. **对比度/饱和度的 `val` 是 1 整数位 + 7 小数位 ⇒ 128 就是 1.0×。** 标定文件里的
    132 / 130 / 128 / 126 就是这个 `val` 的原值，**直接写、不做任何换算**
    （乘 1000 写进去会拿到 `INVALID_ARG`）。
14. **`sc202cs_abs_gain_val_map[]` 有两份同名表**（`sc202cs.c:73` 与 `:475`），
    由 `CONFIG_CAMERA_SC202CS_DIG_GAIN_PRIORITY` 二选一，长度分别是 **197 / 192**。
    我们编进去的是数字增益优先那份 = **192 档**。跨文档引用增益表长度时必须说明是哪一份。
    另有 `sc202cs.c:1167` 的上游 off-by-one（增益下标会越界读），宿主机用例设了护栏。
15. **CCT 是 `rg` 的单变量函数，且在 `rg > 0.78` 处不单调**（2955 K → 3631 K 回折）。
    ⇒ 不在运行期算多项式，改成 **16 点查表 + 提取期强制单调**
    （`rg = 0.7907` 的 CCT 由 3025 K 压到 3009 K，**唯一的人工干预**，脚本会打出来）。
16. **`use_gamma_param = true` 时必须用 γ 解析生成曲线，不能用 JSON 的 `y[16]`。**
    验证过两者**不自洽**：γ=0.5 那档在 `x = 16, 32, …` 栅格上应给出 `64, 90, 111, …`，
    而 JSON 里是 `0, 19, 35, …`，反解出的 x 也不是任何 2 的幂栅格。
17. **`esp_isp_gamma_fill_curve_points()` 用不了** —— 它要函数指针（运行期 `powf`），
    且它的 `y < 256` 检查会拒掉以 256 归一的末点。直接填算好的 `y[16]`。
    驱动的两条硬要求：**每段段长必须是 2 的幂**（我们全用 16）、**末点 x 恰好 255**。
18. **`esp_cam_sensor_set_format()` 必须调**，`sc202cs_detect()` 一个寄存器都没写。
    而 `set_format` 会**重写整张模式寄存器表** ⇒ 读 `0x3902` 必须排在它**之后**。

### 参数在哪调：`main/cam_tune.h`（**只在这里改**）

控制律是纯逻辑、不依赖 ESP-IDF，宿主机可编译可测（`test/test_cam_tune.c` 521 用例、
`test/test_cam_isp_map.c` 532 用例）。
**官方标定值不在这里** —— 它们在机械生成的 `main/cam_isp_cal.h`，`cam_tune.h` 里只放
「我们自己定的东西」与「一行回退的编译开关」。

> ⚠️ 下表里除了 `CAM_CCM_GAIN_*` 三个之外，**每一个值都还没有上过板**。
> 「上板后该翻哪个」见「上板验证清单」。

#### ① 总开关（一行回退，关掉时相关代码整段不编译）

| 宏 | 默认 | 关掉 = 退到什么 |
|---|---|---|
| `CAM_ADN_ENABLE` | 1 | BF / Demosaic 停在寄存器复位值 |
| `CAM_LSC_ENABLE` | 1 | 画面保留四角暗角（省 4368 B 内部 RAM heap） |
| `CAM_AEN_ENABLE` | 1 | 不配 SHARP / Color |
| `CAM_GAMMA_ENABLE` | 1 | **直出线性光** ⇒ 主机按 sRGB 解码会明显偏暗；统计与控制律同在线性域，逆变换一并旁路 |
| `CAM_CCM_MODE` | 1 | **0 = 对角阵** `diag(kr,1,kb)`，只做白平衡、不做色彩还原（T9 之前的形态） |
| `CAM_AE_STAT_ENABLE` | 1 | 不建 ISP AE 统计块、不申请 ISP 中断（AE 只剩软件全帧均值一个口径）。⚠️ `CAM_AE_SOURCE = 1` 时关它会触发 `#error` |
| `CAM_AE_SOURCE` | 1 | **0 = 软件全帧线性均值**（统计块仍建、ρ 仍打，退回后现场还能继续对比） |
| `CAM_AWB_ENABLE` | 1 | 白平衡钉在 `CAM_CCM_GAIN_R/B_MILLI` 那对静态标定值上 |
| `CAM_AWB_STAT_ENABLE` | 1 | 不建 ISP AWB 统计块、帧泵里没有那次 oneshot 阻塞 |
| `CAM_AWB_SOURCE` | 1 | **0 = 软件灰世界 + 四道防护**（增益上限跟着回到 3000） |
| `CAM_HIST_ENABLE` | 1 | 不建直方图统计块；`env.luma` 恒为 0 |
| `CAM_ENV_MODEL` | 1 | **0 = 用实测 ev 断点插值**（`CAM_ENV_EV_BREAKS`）代替官方 `k/ev` 重建式 |
| `CAM_GAMMA_ADAPTIVE` | 1 | **0 = 钉在 `CAM_GAMMA_SLOT`**（默认第 0 档 γ=0.5），逐位回到 `751b24bb` |
| `CAM_BACKLIGHT_ENABLE` | 1 | 恒取高光优先，AE 目标不随场景变 |
| `CAM_LSC_BY_CT` | 1 | **0 = 钉在 `CAM_LSC_SLOT_DEFAULT`**（第 1 档 = 5210 K） |
| `CAM_SAT_BY_CT` | 1 | **0 = 钉在 `CAM_SATURATION_FIXED_VAL`**（128 = 1.000×） |
| `CAM_SENSOR_BLC_ENABLE` | **0** | **默认就是关**（只读 `0x3902` 不写）。翻 1 = 写 `0xc0` 开传感器自带 BLC |

> ⚠️ 三条 `#error` 拦下不相容组合：`CAM_GAMMA_ADAPTIVE` 依赖 `GAMMA_ENABLE` 与
> `HIST_ENABLE`、`CAM_BACKLIGHT_ENABLE` 依赖 `HIST_ENABLE`、
> `CAM_AE_SOURCE=1` 依赖 `AE_STAT_ENABLE`。
> 否则运行期表现是「永远不换档 / 背光永不触发 / AE 拿一份全 0 统计把曝光推到顶」，
> 而自检行还显示「动态 / 已启用」。
...
| `CAM_CCM_STRENGTH_MAX` | **192** | 官方矩阵的强度总闸（/256）。⛔ **192 是降级档不是终值** —— 判定点是黑电平实测，见「上板验证清单」阶段 4 |
| `CAM_CCT_DEFAULT_K` | 5210 | 还没有可信白点统计时用的 CCT（= LSC 中间档的色温） |
| `CAM_SAT_CT_HYST_K` | 150 | 饱和度换档门限（官方 4500 K）两侧的迟滞，防 CCT 在门限上抖 |
| `CAM_SATURATION_FIXED_VAL` | 128 | `CAM_SAT_BY_CT = 0` 时钉住的值（128 = 1.000×） |
| `CAM_LSC_SLOT_DEFAULT` | 1 | `CAM_LSC_BY_CT = 0` 时钉住的档（1 = 5210 K） |
| `CAM_LSC_TRANSPOSE` | 0 | LSC 表 273 个数的排布是 **x 快变还是 y 快变**是 [缺口]。上板若见「暗角修正横竖颠倒」（画面上下亮、左右暗）就改 1 重编，**不要改机械生成的标定表** |
| `CAM_FEEDFWD_HYST_TICKS` | 3 | 前馈级换档迟滞（拍）。**LSC 用它的两倍** —— 273×2 条 LUT 写是本工程最贵的一次重配，而 rev v1.0 没有影子寄存器 |

#### ③ 自动曝光

| 宏 | 默认 | 说明 |
|---|---|---|
| `CAM_AE_TARGET` | **62** | = 官方 `agc.luma_adjust.target`（**线性域**）。>150 会让室内白墙削顶，太低整体发灰 |
| `CAM_AE_TARGET_LOW` / `_HIGH` | **56 / 64** | 官方**非对称**死区（`CAM_AE_SOURCE = 1` 时生效；= 0 时退化成 `目标 ± CAM_AE_DEADBAND` 的对称形式） |
| `CAM_AE_DEADBAND` | 6 | 防振荡闸①（对称形式下用） |
| `CAM_AE_HL_OFFSET` / `_LL_OFFSET` | **−3 / +1** | 官方 `agc.{high,low}_light_priority.luma_offset`，由 `_Static_assert` 钉在标定值上 |
| `CAM_BACKLIGHT_BRIGHT_PCT` | 25 | 背光判据：亮块占比 ≥ 25% **且** 场景均值 < `CAM_AE_TARGET_LOW`。**这两个数是我们定的**，官方选择器闭源 |
| `CAM_AE_DAMP_NUM` / `_DEN` | 1 / 2 | 防振荡闸②。测量到执行有约 170 ms 滞后，增益 1.0 就在振荡边缘 ⇒ **不要改成 1/1** |
...
| 宏 | 默认 | 用在哪条路 | 说明 |
|---|---|---|---|
| `CAM_AWB_MIN_COUNTED` | 1200 | 源 1 | = 官方 `awb.min_counted`。白点数不足 ⇒ 整拍不动，**且 CCT 不更新** |
| `CAM_AWB_RG_MIN/MAX_Q4`、`_BG_*` | 3801 / 8790、2903 / 6587 | 源 1 | 官方白点盒子（1e-4 定点）。判的是**累加之后的重心**（硬件已逐像素筛过） |
| `CAM_AWB_ONESHOT_MS` | 60 | 源 1 | ⚠️ **绝不能传 0**（见上文）。超时在涨就加到 100 试 |
| `CAM_STAT_BLC_PEDESTAL` | **0** | 源 1 | 统计侧黑电平减法。⛔ **刻意留 0** —— 减去一个没量过的基座会主动把 rg/bg 推偏，那不是保守是押注 |
| `CAM_AWB_INTERVAL_TICKS` | 10 | 两条 | AWB 周期 = 1 秒（与 AE 的时间尺度分离） |
| `CAM_AWB_DEADBAND_PCT` | 5 | 两条 | 增益差 <5% 不动 |
| `CAM_AWB_DAMP_NUM` / `_DEN` | 1 / 2 | 两条 | 阻尼。源 1 上它的理由变了：不再是环路稳定性（已经没有环了），而是对**估计噪声**的时域平滑 |
| `CAM_AWB_STEP_MAX_PCT` | 25 | 两条 | 单步限幅 |
| `CAM_AWB_GAIN_MIN_MILLI` | 1000 | 两条 | 下限有物理依据（绿通道恒为最强） |
| `CAM_AWB_GAIN_MAX_MILLI` | **3445**（源 1）/ 3000（源 0） | 两条各自 | 3445 = `10⁷/2903`，官方白点轨迹端点给出的 kb 最大值（2289 K 白炽灯）。留在 3000 会在暖光下顶住 ⇒「白炽灯下永远偏黄」 |
| `CAM_AWB_LUM_MIN` / `_MAX` | 20 / 200 | **仅源 0** | 源 1 上被硬件的 `white_patch.luminance` 窗（164~533）取代，两个计数器恒为 0 |
| `CAM_AWB_CAST_MAX_PCT` | 200 | **仅源 0** | 单色场景防护。源 1 上被官方白点框取代，`SKIP_CAST` 恒为 0 |
| `CAM_AWB_CONVERGE_TICKS` | 3 | 两条 | 纯观测量 |

#### ⑤ 直方图与环境亮度

| 宏 | 默认 | 说明 |
|---|---|---|
| `CAM_HIST_INTERVAL_TICKS` | 10 | 直方图 oneshot 周期 = 1 秒 |
| `CAM_HIST_PHASE` | **5** | 与 AWB 的 oneshot **错开半个周期**。⚠️ 这是**帧泵预算**：任何一拍里最多一次 60 ms 阻塞，100 ms 的固定节拍才不会被 120 ms 吃掉。**改任一周期时两个数要一起看** |
| `CAM_HIST_ONESHOT_MS` | 60 | 同 AWB |
| `CAM_ENV_EV_BREAKS` | `{16556, 8306, 2775, 833}` | `CAM_ENV_MODEL = 0` 时用的 ev 断点。默认这四个是用官方 `k = 250000` 反推的 ⇒ **翻开关本身不改变行为**，改变只来自把断点换成实测值 |
...
**从上往下读，第一条不对的就是根因。** 三组，**顺序就是排查顺序**：

| 组 | 行 | 回答什么 |
|---|---|---|
| ① `camera:` 取流 | `SCCB` / `CSI` | **有没有帧** |
| ② `camera:` 画质与闭环 | `画质` / `CCM` / `黑位` / `AWB` / `AWB统计` / `AWB对照` / `AE` / `AE统计` ×2 / `HIST` ×2 / `ENV` | **帧对不对** |
| ③ `camera:` 前馈画质级 | `ADN` / `LSC` / `AEN` / `GAMMA` | **画质级配上没有**（既不影响有没有帧，也不参与两个闭环 ⇒ 排最后） |
| ④ `uvc:` | `streaming` / `编码` / `缩放` / `画面` | **帧有没有发出去** |

每一行都必须能分出**四种情况**（别用同一个哨兵表达其中两种）：

| 打出来的 | 含义 |
|---|---|
| `未编译` | 对应开关 = 0，整段代码不在镜像里（由 `#else` 分支那行说出来） |
| `未运行` | 编译进来了但一次都没配过 / 没跑过 |
| `<错误码>` | 配了、硬件或驱动拒了 ⇒ 打的是 `esp_err_to_name()`，直接可查 |
| `ESP_OK` | 配上了 ⇒ **同时打出此刻硬件里的关键参数值**，可与官方标定表逐个核对 |
...
camera: [自检] 画质 CCM=ESP_OK 当前 R×1.700 G×1.000 B×1.550
             | 通道均值(线性) R98 G121 B105 → 建议 R×2.099 B×1.786（对白纸/灰卡时才作数）
camera: [自检] CCM 模式=官方表 ESP_OK CCT=5210K(估计) 档11/19(5210K→5476K w=0.00)
             强度=192/256(定点可行 t*=256，总闸 192) 白平衡 R×1.700 B×1.550
             | 下发 [...] max|v|=1.842(上限 3.990) | 重配=3 换档=ESP_OK
camera: [自检] 黑位 线性最暗 lum=2 r=1 g=2 b=1（官方 acc.blc=16；**盖住镜头 + AE 顶格**时才是黑电平）
             | 传感器BLC=关(只读不写) 0x3902 读=ESP_OK 写前=0x80 写=未写 写后=未写
camera: [自检] AWB=开 R×1.700 B×1.550 调整中 下发=0 最近=白点太少(未运行)
             | 未更新：AE未稳=0 暗场=0 过亮=0 色偏过大=0 白点太少=12 未到周期=0 死区内=0 增益越界=0 量化无变化=0
camera: [自检] AWB统计=ESP_OK 最近=ESP_OK 触发=12 超时=0 | 白点=41232/921600(4.4%)
             平均R+G+B=319(硬件窗 164~533) 平均G=143(官方 green 98~210)
             | Σr/Σg=0.5882 Σb/Σg=0.6452 → CCT=5210K 绝对增益 R×1.700 B×1.550
camera: [自检] AWB对照 硬件白点(CCM前·绝对) R×1.700 B×1.550
             | 软件灰世界(CCM后·当前×g/x) R×2.099 B×1.786 | 差 R19.0% B13.2% | AWB 源=…
camera: [自检] AE=ESP_OK 源=硬件5×5(官方加权,demosaic后) 曝光=624/1244 增益=1.000×(第 0 档)
             曝光量=624 | 亮度 60→目标 59[53,61]（基准 62，优先级=高光，偏移 -3）已收敛
             下发=4 最近=ESP_OK | 背光=否(亮块>=25% 且 场景均值<56) 切换=0
camera: [自检] AE统计=ESP_OK 运行=ESP_OK 帧=300 | 硬件加权=60（暗块 0/5 亮块 0/3）
             软件线性=76 ρ=0.789（=硬件/软件，预测 0.79）| AE 源=本统计（已接管）
camera: [自检] AE统计 25 块: ...25 个彼此不同的数...
camera: [自检] HIST=ESP_OK 最近=ESP_OK 触发=12 超时=0 | 场景均值=74（均匀权重，与 AE
             的中心加权是两个量）亮块=8% 暗块=21% | Σbin=921600(应为 921600)
camera: [自检] HIST bin(每格 16 个码值): ...16 个数...
camera: [自检] ENV 模型=官方 k/ev（[反推]，非确证）ev=624 场景均值=74 AE目标=59
             → env.luma=400.6 → 想要 gamma 档=3 | 断点 15/30/90/300 量程=[12.5, 400.6]
             用过的档 --23 想换档=2
```

> ⚠️ **上面这段是格式示例，数字是编的** —— 本轮没有上过板，
> 一个真实读数都还没有。判读判据见每一行下面的表与「上板验证清单」。
...
### 上板验证清单（ISP 对齐那一轮，**十个 commit 叠在一起，按这个顺序验**）

> **为什么要有这份清单**：`751b24bb` … `e6201fc3` 十个 commit 全部只经过宿主机验证，
> **一次都没烧过板**。它们改动了 gamma、AE 目标、AE 反馈量来源、AWB 估计器与公式形态、
> CCM 形态、外加五个前馈画质级 —— 出问题时「画面不对」这一个现象能对应十几种病因。
>
> **原则：从底层往上。** 先验**统计抽头是不是真的在看画面**（阶段 2），
> 再验**控制律收不收敛**（阶段 3），最后才验**观感**（阶段 5）。
> 顺序颠倒的话，一个「抽头压根没配上」会被误读成「控制律参数不对」，然后越调越远。
>
> **三个判定点**用 ⭐ 标出：它们各自决定后面一整段能不能成立。
...
python3 test/isp_cal_extract.py --check           # 判据：md5 与 JSON 一致
cd test && for t in touch_map kbd_translate standby_screen audio_frame uac_volume \
                    uvc_pattern cam_frame_stats cam_tune cam_isp_map; do
...
#### 阶段 1：开机基本面 —— 链路还在不在（5 分钟）

先确认这十个 commit **没有打断任何一条已验证的链路**。摄像头**先不开**。

| # | 检查 | 判据 | 不过 ⇒ |
|---|---|---|---|
| 1.1 | 开机日志 | `[自检] CSI` 六步全 `ESP_OK`；`FIFO: EP4 IN=112 words` 出现 | 与 ISP 对齐无关，回去看 USB / CSI 那两章 |
| 1.2 | host `lsusb` | `16d0:10a9`，IN 端点恰为 `0x81/0x82/0x83/0x84` | 同上 |
| 1.3 | GUD 显示 | `/dev/dri/cardN` 存在，`modetest` 出图 | 同上 |
| 1.4 | HID 键盘 / 触摸 | `evtest` 收到按键与 5 点绝对坐标 | 同上 |
| 1.5 | UAC1 | `aplay` 出声、`arecord` 有波形 | 同上 |
| 1.6 | 前馈级五行自检 | `ADN=` / `LSC=` / `AEN=` / `GAMMA=` / `CCM 模式=` 全是 `ESP_OK`（不是错误码、不是「未运行」） | 见下表 |

前馈级建不起来时的归因（**只降级不拦启动**，所以链路照常，画面不对）：

| 现象 | 归因 | 处置 |
|---|---|---|
| `LSC=ESP_ERR_NOT_SUPPORTED` | ⛔ **芯片版本低于 v1.0** ⇒ 本工程关于 LSC 的全部前提塌掉 | 读 `esptool.py chip_id` 确认，回来改 README 的版本表；`CAM_LSC_ENABLE = 0` |
| `LSC=ESP_ERR_INVALID_SIZE` | 网格数 ≠ 273 ⇒ ISP 分辨率变了 | 标定表与分辨率对不上，不要硬填 |
| `ADN=ESP_ERR_INVALID_ARG` | BF 的 padding tail valid 参数（两个都得是 0） | — |
| `AEN=ESP_ERR_INVALID_ARG` | Color 的 `val` 超 255 ⇒ 多半把 `1.031` 乘了 1000 | — |
| 任一格是 `esp_err_to_name()` 认不出的码 | `intr_priority` 与处理器不一致 ⇒ 驱动返回的是**布尔 1** | 三个统计块与处理器全写 0 |
| `GAMMA` 那行 `统计域=gamma 域(逆表未建)` | gamma 配失败 ⇒ 逆表跟着不建（两侧自洽，不是 bug） | 先修 `GAMMA=` 那格的错误码 |

#### 阶段 2：统计抽头是不是真的在看画面（**纯观测，此时不看控制律**）

> 这一阶段的全部意义：**一块没被写过的零缓冲，在计数器上与真实画面一模一样。**
> 下面每一条都是「让画面动，看统计跟不跟着动」的可证伪判据。
> 打开 `ffplay` 让摄像头取流，然后照做。

| # | 抽头 | 动作 | 判据 | 不过 ⇒ 翻哪个 / 退到哪 |
|---|---|---|---|---|
| 2.1 | **AE 5×5** | 看 `[自检] AE统计 25 块` | 25 个数**彼此不同**（全 0 = 窗口 `bsize = 0`；全都相同的非零值 = 分块没生效） | `CAM_AE_SOURCE = 0` ⇒ 退到 `93f95c6c`（T5）的控制行为 |
| 2.2 | AE 5×5 | 只遮**半边**镜头 | **只有一侧的块掉下去**（顺带把硬件块排布方向测出来，把结论记进 `cam_on_ae_stat` 的注释） | 同上 |
| 2.3 | AE 5×5 | 手电照中心 | 中心块 > 250、`亮块` 计数 ≥ 3 | 同上 |
| 2.4 | **HIST** | 遮住镜头 | `暗块 ≥ 80%`、16 个 bin 整体左移；`Σbin == 921600` | `CAM_HIST_ENABLE = 0` ⇒ 退到 `8d9e68c3`（T7）；连带 `CAM_GAMMA_ADAPTIVE` / `CAM_BACKLIGHT_ENABLE` 被 `#error` 拦住，一起关 |
| 2.5 | **AWB 白点** | 拍**白纸**（充满画面） | `白点占比 > 30%` | 见 2.7 |
| 2.6 | AWB 白点 | 拍**红墙**（或任何单色物体） | `白点占比 < 5%` | 见 2.7 |
| 2.7 | AWB 白点 | 读同一行 | `平均G` 落在 `[98, 210]`、`平均R+G+B` 落在 `[164, 533]`、`CCT` **不**恒定贴在 2289 或 7466 | 白点恒 0 ⇒ 临时把 rg/bg 放宽到 `[0.1, 3.9]`、亮度窗放宽到 `[1, 764]` 重测：**这时若有白点，说明官方框用错了域** ⇒ `CAM_AWB_SOURCE = 0`，L3 只做固定 CCT 的 CCM / LSC / 饱和度 |
| 2.8 | **LSC** | 读 `[自检] LSC` 的五个数 | **中心 ≈ 256（1.00×）、四角 700~860**；四个角彼此接近 | 四角彼此差很多 / 某角接近 256 ⇒ **排布顺序猜错了** ⇒ `CAM_LSC_TRANSPOSE = 1` 重编（**不要改标定表**）；仍不对 ⇒ `CAM_LSC_ENABLE = 0` 退到 `d8ce8ca0`（T2） |
| 2.9 | **gamma 逆表** | 读 `[自检] GAMMA` 并排的两个均值 | `gamma后均值 ≥ F(线性均值)`（詹森不等式，F 凹），且两者**明显不同** | 两者 ≈ 相等 ⇒ **逆表没生效**；`gamma后 明显 < F(线性)` ⇒ **逆表与硬件曲线不同源**（最该怕的一种：画面看着正常，但 AE/AWB 在拿偏移量闭环）⇒ `CAM_GAMMA_ENABLE = 0` |
| 2.10 | 帧统计本身 | 读 `[自检] 画面` | `最暗 ≠ 最亮`、**校验和帧帧在变**、`下 1/8 历史最亮` 不恒为 0 | 这三条不过说明取到的不是真实画面，上面全部作废 |

⭐ **判定点 ①：ρ ≈ 0.79** —— 决定 `CAM_AE_SOURCE = 1` 整段论证成不成立。

```
camera: [自检] AE统计=… | 硬件加权=NN（暗块 x/5 亮块 y/3）软件线性=MM ρ=0.NNN（=硬件/软件，预测 0.79）
```

- **测法**：普通室内场景，**等 AE 收敛**（`已收敛`），反复三次遮挡/复原，读三次 ρ。
- **判据**：三次都回到同一个值（±0.03），且该值 ≈ **0.79**。
- **ρ 只在 AE 已收敛时才有意义** —— 曝光正在大幅调整时两个口径采的是不同瞬间的画面。
- **显著偏离 0.79 ⇒ 「硬件采样点在 CCM 上游」这条推导错了**，
  那会推翻 `CAM_AE_SOURCE = 1` 的全部依据 ⇒ **`CAM_AE_SOURCE = 0`**，退到 `93f95c6c`（T5）。
  ⓘ ρ 的预测值来自 `1/(0.30·kr + 0.586 + 0.113·kb) = 1/1.271`，
  `kr`/`kb` 就是「画质」那行实时打着的两个数 —— 白平衡调到别的值时，预测值要跟着重算。

⭐ **判定点 ②：`env.luma` 重建站不站得住** —— 决定 `CAM_GAMMA_ADAPTIVE` 能不能用。

```
camera: [自检] ENV 模型=官方 k/ev（[反推]，非确证）ev=… → env.luma=… → 想要 gamma 档=…
              | 断点 15/30/90/300 量程=[…, …] 用过的档 0-2- 想换档=N
```

- **测法**：从**明亮日光**（窗边/户外）走到**昏暗室内**，中间停留至少 3 秒让 AE 收敛。
- **判据**：「**用过的档**」那个位图里至少出现**两个**不同的数。
- **不过**（四档只用得到一档，或 `env` 恒在 3001 以上 / 151 以下）⇒ **重建式是错的**：
  `CAM_ENV_MODEL = 0`，并把 `CAM_ENV_EV_BREAKS` 换成**实测的四个 ev**
  （记下是哪种光照、AE 收敛后的 ev 是多少）。
  实在不成 ⇒ `CAM_GAMMA_ADAPTIVE = 0`，gamma 钉回第 0 档（逐位回到 `751b24bb`）。
- 「想换档」随时间**线性增长** ⇒ 迟滞不够，加大 `CAM_CAL_GAMMA_MIN_STEP_Q1`（官方 30）并记录理由。

#### 阶段 3：控制律收不收敛

前提：阶段 2 全过。抽头没验就调控制律 = 在猜。

| # | 环 | 动作 | 判据 | 不过 ⇒ |
|---|---|---|---|---|
| 3.1 | **AE** | 遮挡 / 复原，各三次 | 三次都收敛（`已收敛`），全程**单调不过冲**，最坏 ≈ 6 步（2 秒） | 振荡 ⇒ 加大 `CAM_AE_INTERVAL_TICKS` 或减小阻尼；顶限位（亮度长期偏离而 `下发` 不涨）⇒ 加光或抬 `CAM_AE_GAIN_MAX_MILLI`（代价是噪声） |
| 3.2 | **AWB 对照** | 普通场景，AE 收敛后 | `[自检] AWB对照` 的「差 R/B」**< 10%**，且两个估计器**方向一致**（不能一个 >1 一个 <1） | 方向相反 ⇒ **rg 的分子分母颠倒**或采样点没配上（此时 CCT 会同时贴在端点）⇒ `CAM_AWB_SOURCE = 0` |
| 3.3 | ⭐ **AWB 发散** | **静置 5 分钟**，光源不动 | 「死区内」持续涨而「**下发**」**不涨**；增益**不动** | ⛔ 增益单调爬到 3.445 或掉到 1.000 ⇒ **绝对公式被当成增量公式用了** ⇒ **立刻 `CAM_AWB_SOURCE = 0` 重烧**，退到 `7679189a`（T8）的控制行为 |
| 3.4 | AWB 收敛 | 换光源（白炽 ↔ 日光） | 增益单调走到新值并停住，`下发` 不再涨；`白点太少` 在单色场景下涨是**正确行为** | 来回摆 ⇒ 加大 `CAM_AWB_INTERVAL_TICKS` 或减小阻尼 |
| 3.5 | **CCM 换档** | 换光源 | `重配=` 每秒 **≤ 1**；`档 i/19` 跟着 CCT 走；`max\|v\| ≤ 3.990` | `重配` 每秒 > 1 ⇒ 迟滞没起作用 / CCT 在档边界抖；`max\|v\| > 3.990` ⇒ **钳制没生效**（阈值被改成 4000？）⇒ 此时硬件里的矩阵与日志打的**不是同一个** ⇒ `CAM_CCM_MODE = 0` |
| 3.6 | **CCM 行和** | 拍白纸，AE 收敛 | 三通道均值差 **< 5%** | 破了 ⇒ 先看档号是不是插到了两端那两个单位阵档（1200 K / 12000 K）上 |
| 3.7 | **LSC / 饱和度换档** | 换光源 | 白炽灯 ⇒ 档 0（2410 K）；日光 ⇒ 档 2（8200 K）；室内白光 ⇒ 档 1。饱和度在 128 / 130 之间**不抖** | 档一直是 1 且 `CCT=…(默认)` ⇒ **预期**（白点数没到 `min_counted`，CCT 冻在默认值），去看 2.5–2.7；饱和度抖 ⇒ 加大 `CAM_SAT_CT_HYST_K`；`换档=` 线性增长 ⇒ CCT 在两档中点（3810 / 6705 K）附近抖 |
| 3.8 | 换档撕裂 | 换光源的**那一瞬** | 画面**不闪暗角** | 闪 ⇒ 加大 `CAM_FEEDFWD_HYST_TICKS`（LSC 用它的两倍）。**仍在则接受**：rev v1.0 没有影子寄存器，273×2 条 LUT 写落在帧内就是硬件限制 |
| 3.9 | 前馈级跟不跟增益 | 遮住镜头让 AE 顶到高增益 | BF 档 0 → 3~6、`level` 2 → 8~10；SHARP 档 0 → 3、`m系数` 1.525 → 1.225；对比度 132 → 126。开灯回落 | 档号一直不动 ⇒ 看「增益=」那格是不是喂错了值 |

#### 阶段 4：⭐ **判定点 ③ —— 黑电平基座**（决定 CCM 强度参数）

这是 `CAM_CCM_STRENGTH_MAX = 192` 这个降级档唯一的解除条件。

**测法（顺序不能变）**：

1. **完全盖住镜头**（不是关灯，是物理遮死，别漏光）；
2. **等 AE 顶到上限**（`[自检] AE` 的曝光 = `exp_max` 且增益 = `CAM_AE_GAIN_MAX_MILLI`）；
3. 读 `[自检] 黑位` 那行的**线性域**四个最暗值。

> ⚠️ 必须是**线性域**：黑电平是加在 RAW 上的加性常数；而 gamma 在暗部扩张最厉害
> （γ=0.5 档首段斜率 4）⇒ 线性 16 会被编码成 64。拿画面域的 `lum_min` 去和官方的 16 比
> **会差四倍**，这个判定当场失效。

| 读数 | 结论 | 动作 |
|---|---|---|
| `r/g/b` 都 ≤ 3 | 基座 ≈ 0 | ✅ **放行**：`CAM_CCM_STRENGTH_MAX = 256`（用满） |
| 10~20 且三者相近 | 基座 ≈ 16（= 官方 `acc.blc`） | 先把 `CAM_SENSOR_BLC_ENABLE = 1` 重烧再测一次；仍 ≈16 ⇒ **降级放行**：保持 192 **且** `CAM_STAT_BLC_PEDESTAL = 16` |
| 彼此差 > 5 | 基座自带色偏，或被 LSC 放大 | 先 `CAM_LSC_ENABLE = 0` 隔离，重测 |
| > 30 | **漏光/没盖严** | 重新盖，**别急着写寄存器** |
| `0x3902 写前 = 0xc0` | 传感器 BLC 上电就开着 | 基座本该 ≈ 0 |
| `0x3902 写前 = 0x80` | 关着 | 预期基座 ≈ 16 |
| `写后 != 写入值` | 这一位不可写（只读/保留） | **这条路走不通**，`CAM_SENSOR_BLC_ENABLE` 改回 0 |

⛔ **在拿到这个数之前，不要把 `CAM_CCM_STRENGTH_MAX` 调上去。** 那等于在一个未知的基座上
做大动态矩阵运算，画面变化无法归因。
...
| # | 检查 | 判据 |
|---|---|---|
| 5.1 | 整体亮度 | `ffplay` 里的画面曝光正常，不是「几乎全黑」也不是削顶发白 |
| 5.2 | 颜色 | 人脸不是蓝的（没有 R/B 互换）；白纸是白的；红色物体是红的 |
| 5.3 | 暗角 | 拍白墙，四角/中心亮度差 **< 15%**（改动前约 0.3~0.4 倍） |
| 5.4 | 噪声 | 暗处噪点**不比对角阵 CCM 时明显** —— 官方 CCM 的非对角项会放大高增益噪声。明显变差 ⇒ 提高 BF 档断点或降 `CAM_CCM_STRENGTH_MAX`（都是一行改动） |
| 5.5 | 锐化 | 边缘**没有**白边/黑边（过锐 ⇒ h 系数的定点换算错了，整数位/小数位颠倒）；高增益下噪点没被锐化成明显颗粒 |
| 5.6 | 方向 | 画面不上下颠倒、不左右镜像 |

#### 阶段 6：复合回归

见文末的「**五项能力复合回归规程**」一节。**这是 P0 / P3 欠账的采集时机**，不要单独再跑一次。

#### 「全部开关关掉 = 退回哪个状态」

| 关掉 | 退回哪个 commit 的行为 |
|---|---|
| `CAM_LSC_BY_CT` / `CAM_SAT_BY_CT` | `2b7f7d3b`（T12）：LSC 钉 5210 K 档、饱和度钉 128 |
| `CAM_GAMMA_ADAPTIVE` | `751b24bb`：gamma 钉第 0 档（γ=0.5），**逐位相同** |
| `CAM_ENV_MODEL` | **不改变行为**（默认断点就是用官方 k 反推的，两条路逐点重合）；改变只来自把断点换成实测值 |
| `CAM_HIST_ENABLE` | `8d9e68c3`（T7）。⚠️ 连带 `GAMMA_ADAPTIVE` / `BACKLIGHT_ENABLE` 被 `#error` 拦住，必须一起关 |
| `CAM_CCM_MODE` | `7f2c4f26`（T9）：对角阵 `diag(kr,1,kb)` |
| `CAM_AWB_SOURCE` | `7679189a`（T8）：软件灰世界 + 四道防护（增益上限跟着回到 3000） |
| `CAM_AWB_STAT_ENABLE` | `8d9e68c3`（T7）：不建 AWB 统计块。⚠️ `AWB_SOURCE=1` 时会 `#error` |
| `CAM_AE_SOURCE` | `93f95c6c`（T5）：软件全帧线性均值（统计块仍建、ρ 仍打） |
| `CAM_AE_STAT_ENABLE` | `751b24bb`：不建 AE 统计块 |
| `CAM_AEN_ENABLE` | `e412fcae`（T3）：不配 SHARP / Color |
| `CAM_LSC_ENABLE` | `d8ce8ca0`（T2）：保留四角暗角 |
| `CAM_ADN_ENABLE` | `751b24bb`：BF / Demosaic 停在寄存器复位值 |
| `CAM_GAMMA_ENABLE` | `43288ae9` 的**输出传递函数**（直出线性光） |

> ⚠️⚠️ **「全部开关关掉」并不等于回到唯一实机验证过的那一版。**
> `CAM_AE_TARGET`（62）、`CAM_AE_DEADBAND`（6）、`CAM_AWB_LUM_MIN`（20）
> **不在任何开关后面** —— 它们是常量。要逐位回到 `43288ae9`（**唯一实机验证过的画质配置**：
> 对角阵 CCM `diag(1.7, 1, 1.55)` + 软件全帧均值 AE + 灰世界 AWB，无 gamma / 无 LSC /
> 无 BF / SHARP / Color / 无任何硬件统计块），除了把上表全部关掉之外，还要把这三个数
> 分别改回 **120 / 12 / 40**。
>
> ⓘ 最省事的「已知好状态」是直接 `git checkout 43288ae9 -- components/packages/tab5-all-in-one/firmware/`
> 重烧一版，用它做 A/B 对照 —— 十个 commit 叠在一起时，**有一个能对比的基准比任何日志都值钱**。
...
```bash
cd firmware/test
for t in cam_tune cam_frame_stats cam_isp_map; do
  cc -std=c11 -Wall -Wextra -Werror -I../main test_$t.c ../main/$t.c -o /tmp/$t && /tmp/$t
done
#   cam_tune        OK (521 cases)
#   cam_frame_stats OK (37 cases)
#   cam_isp_map     OK (532 cases)

# 标定表与官方 JSON 的一致性（**改一个数字就失败**）；需先 idf.py build 把
# managed_components/ 拉下来，JSON 本身不在仓库里
cd .. && python3 test/isp_cal_extract.py --check
```
...
| 测试 | 用例数 | 盯什么 |
|---|---|---|
| `test_touch_map.c` | 32 | 触摸坐标反变换与报告装填 |
| `test_kbd_translate.c` | 16 | 按下集合 → HID 分层翻译 |
| `test_standby_screen.c` | 35 | 待机画面版式（含 PPM 预览） |
| `test_audio_frame.c` | 13 | USB 单声道 ↔ I2S 立体声 |
| `test_uac_volume.c` | 32 | UAC1 音量 ↔ 硬件百分比换算 |
| `test_uvc_pattern.c` | 2040 | 合成图案（已不在固件 SRCS 里） |
| `test_cam_frame_stats.c` | 37 | 帧统计纯函数（含逆 gamma 还原的顺序） |
| `test_cam_tune.c` | 521 | AE / AWB 控制律 |
| **`test_cam_isp_map.c`** | **532** | **ISP 对齐的全部纯逻辑**：定点换算、带迟滞的档位跟踪、CCT 查表、CCM 插值与钳制、5×5 加权测光、gamma 正/逆表、直方图归约、`env.luma` 重建、最近邻选档 |

`test_cam_isp_map.c` 里几条**承重**的不变式（改标定层时先被它们拦住）：

- **CCM 行和恒为 1000**（截断误差 ≤ 2）⇒ 中性面出来仍中性、整体增益精确为 1；
- **中性面三分量相对差 ≤ 1%**（实测最大 0.603%）；
- 任意输入 `max|out| ≤ 3990`；**可行域恰为 `[0, t*]`**，二分给出的就是最大可行 `t`；
  `t_max = 0` 时逐元素退回对角阵；
- gamma：端点、精确穿过每个标定节点、单调、近似凹（官方 y 表取整 ⇒ 第 2 档有一处
  1/16 的斜率抖动，第 0 档严格凹单独断言）、往返误差 `|inv(F(x))−x| ≤ 1`、
  **逐档逆表必须互不相同且交叉档误差 > 5**（挡「换档时逆表没跟着换」）、
  以及「**逆的是折线不是解析式**」的回归钉；
- AWB 硬件路的 **200 拍不动点**：停在 `1/rg`、理由是「死区内」、`SKIP_QUANT` 恒为 0、
  从**高于**不动点的初值出发必须**往下**走（后两条才抓得住「误用增量公式」）；
- `cam_env_luma_from_ev()` 的降级路径与官方 `k/ev` 模型**逐点重合**
  ⇒ 翻 `CAM_ENV_MODEL` 本身不改变行为。

`test_cam_tune.c` 的 **521 个用例**分若干组，值得点名的几组：
...
- **目标平移组**：17 个目标 × 死区四个边界，钉住「死区跟着目标平移、且**不对称是死区的
  性质**（−6/+2），不是 62 这个数的性质」；外加背光判据的与逻辑四条与方向性
  （暗部优先的曝光量必须更大）。
...
>
> 1. **复合回归本身** —— 五项能力同跑 10 分钟，证明 ISP 对齐那一轮没有动到任何一项已验证能力；
> 2. **P0 欠账** —— 脏矩形 / LZ4 的定量验证、**GUD 帧率实测**（含开/关摄像头的对比）；
> 3. **P3 欠账** —— UAC1 全双工长时稳定性、**无反馈端点的时钟漂移**。

### 前置

- 阶段 0–5 的「上板验证清单」已经过（至少阶段 0–2）。摄像头链路不通时这一场没有意义。
- 主机侧准备：
...
- 固件日志里 AE / AWB / HIST 三个统计的 `触发=` **不涨**（VideoStreaming 停在 alt 0）。
...
| 7 | AE 闭环 | 10 分钟里遮挡/复原三次，**三次都收敛**且单调不过冲 | ⏳ |
| 8 | AWB 闭环 | 增益**不单调发散**（判定点 ③.3）；`白点太少` 在单色场景下涨是正确行为 | ⏳ |
...
| 10 | 依赖树 | `ls managed_components \| wc -l` == **12** | ✅ 已验 |
| 11 | UVC 帧率 | 连续 10 分钟：`实测 fps ≥ 9.0`、`拒收 = 0`、`丢弃 = 0`、`抢缓冲 = 0`、`取帧超时 = 0`、`帧长不符 = 0` | ⏳ |
| 12 | alt 0 零占用 | 步骤 C 之后：`PSRAM 读 停流后` 回到 `bw_idle` **同量级**；AE / AWB / HIST 三个 `触发/帧` 计数**不再涨** | ⏳ |
| 13 | JPEG 预算 | 平均帧 ≤ 30 KB、**峰值 ≤ 44 KB** | ⏳ |
| 14 | 内存 | `esp_get_free_internal_heap_size()` 相对对齐前减少 ≈ **4.4 KB**（LSC 的 4×273×4 = 4368 B 是从内部 RAM heap 分配的），PSRAM 无变化 | ⏳ |
...
| `[自检] 编码 … 失败=` | **0** |
| `[自检] streaming … 拒收=` | **0** |
| `[自检] AWB统计 … 超时=` / `[自检] HIST … 超时=` | **0** —— 非 0 说明两个 oneshot 挤掉了帧泵预算（先看 `CAM_HIST_PHASE` 的错相有没有生效，再把周期拉到 2 秒，最后才考虑把触发挪到 USB 提交之后） |
...
| `main/cam_frame_stats.{c,h}` | 帧统计纯函数（亮度均值/最暗/最亮/分通道均值/**线性域四个最暗值**/FNV-1a 校验和 + 逐像素逆 gamma 还原），零依赖 |
| `main/cam_isp_cal.h` | **官方标定表（机械生成物，不许手改）**：BF 7 档 / Demosaic 4 档 / SHARP 4 档 / gamma 4 档 / CCM 19 档 / LSC 3 档 × 4 通道 × 273 格 / AWB 白点框与白点轨迹 16 点 / AE 5×5 权重与阈值。来源 `sc202cs_default.json`（Apache-2.0，随 `esp_ipa` 分发，**不在仓库里**） |
| `main/cam_isp_map.{c,h}` | **ISP 对齐的全部纯逻辑**：定点换算（进位 + 溢出检测）、带迟滞的档位跟踪、按增益/色温选档、CCT 查表、CCM 逐元素插值与「朝单位阵混合」钳制、5×5 加权测光 + quorum 剔除、gamma 正/逆表生成、直方图归约、`env.luma` 重建。零 ESP-IDF 依赖（宿主机可测） |
| `test/test_cam_isp_map.c` | 上述纯逻辑的宿主机回归（**532 用例**） |
| `test/isp_cal_extract.py` | 从官方 JSON 机械提取标定表 → `cam_isp_cal.h`；`--check` 是烧板前的闸门（md5 + 字节数双指纹） |
...
| `test/test_cam_tune.c` | AE/AWB 控制律的宿主机回归（**521 用例**，含单色场景、带滞后的闭环仿真、硬件路的 200 拍不动点、目标平移组） |
| `test/test_cam_frame_stats.c` | 帧统计纯函数的宿主机回归（37 用例） |
