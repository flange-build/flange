/*
 * M5Stack Tab5 面板 init 命令序列（两种批次各一份）。
 *
 * vendor 自 esp-bsp，逐字节原样搬运，未做任何改动：
 *   ILI9881C ← bsp/m5stack_tab5/priv_include/disp_init_data.h
 *   ST7123   ← bsp/m5stack_tab5/priv_include/disp_init_data_1.h
 *
 * SPDX-FileCopyrightText: 2025 Espressif Systems (Shanghai) CO LTD  （ILI9881C 部分）
 * SPDX-FileCopyrightText: 2026 Espressif Systems (Shanghai) CO LTD  （ST7123 部分）
 * SPDX-License-Identifier: Apache-2.0
 *
 * 必须在 esp_lcd_ili9881c.h 与 esp_lcd_st7123.h 之后包含：两个数组的元素类型
 * ili9881c_lcd_init_cmd_t / st7123_lcd_init_cmd_t 由那两个头文件定义。
 *
 * 这是硬件寄存器序列，改一个字节就是一个新 bug。不要重排、紧凑化或删空行——
 * 保持与上游逐行可 diff，将来 esp-bsp 更新时才好对照。
 *
 * 注意：ST7123 那份未经实机验证。本机实测内部 I2C 扫到 0x14(GT911)、无 0x55，
 * 属 ILI9881C 批次，走的是上面那份；ST7123 分支只编译不执行。
 */
#pragma once
