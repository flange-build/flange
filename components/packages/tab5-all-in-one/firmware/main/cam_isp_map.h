...
/* ── gamma：前向曲线求值 与 逆查表 ─────────────────────────────────
 *
 * ⚠️ **被逆的是硬件那条 16 点折线，不是解析式 γ。** 硬件（isp_ll.h 的
 *   isp_ll_gamma_set_correction_curve）只存 16 个 y 与每段的 log2 段长，段内**线性
 *   插值** —— 它与 y = 255·(x/255)^γ 在第一段上差到 **16 级**（γ=0.5 档，x=1 处
 *   解析式给 16、折线给 4）。统计侧要还原的是**画面里真实发生过的那个变换**，
 *   所以这两个函数都以折线为准；套解析式去逆等于在暗部引入一个 16 级的系统偏差，
 *   而暗部恰好是 AE/AWB 最吃紧的地方。
 *
 * 节点定义（与 isp_ll 逐位对齐）：(0,0) 之后是标定表的 16 个点，
 * **末点的 x 按硬件语义取 256**（驱动要求 pt[15].x == 255，但段长按 256−240 = 16
 * 这个 2 的幂来编码，见 isp_gamma.c 的 x_delta 与 isp_ll 的 __builtin_ctz）。
 *
 * slot 越界时钳到末档（不返回错误：调用方拿它去查表，给一条合法曲线比给一个
 * 需要处处判断的错误码安全）。
 */

/* 前向：线性值 x → gamma 编码值。恒单调非减，F(0) = 0。 */
uint8_t cam_gamma_forward(uint32_t slot, uint8_t x);

/*
 * 逆表：lut[g] = 使 F(x) ≈ g 的线性值 x。256 项，恒单调非减，lut[0] = 0。
 *
 * 精度不是对称的，判读时要知道：曲线在暗部**扩张**（γ=0.5 档首段斜率 4）⇒ 逆回去
 * 几乎无损；在亮部**压缩**（末段斜率 0.5）⇒ 一级 gamma 值对应两级线性值。
 * 宿主机测试守着的界是 |lut[F(x)] − x| <= 1、|F(lut[g]) − g| <= 2（四档全扫）。
 *
 * lut 不可为 NULL。
 */
