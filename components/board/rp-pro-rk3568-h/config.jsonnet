// RP Pro RK3568H 板级配置：声明设备树、产品变体和板载硬件策略。
...
// 不同总线，无冲突，无须 blacklist 或 patch 关闭 in-tree。
{
  ...
  // overlays.board = 编译进 boot.img；overlays.enabled = extlinux 默认加载。
  boot+: {
    overlays+: {
      board: ['rk3568-pro-rk3568-h-spi0-to-i2c2.dtbo'],
      enabled: ['rk3568-pro-rk3568-h-spi0-to-i2c2.dtbo'],
    },
  },
  kernel+: {
    ...
    //
    // builder 按 kernel.device_tree.directory/name 推导源 DTB；directory 可含
    // 子目录，name 必须保持不带斜杠的 basename，收集后按 basename 平铺。
    device_tree+: { directory: 'rockchip/rp-rk356x', name: 'pro-rk3568-h' },
  ...
  rootfs+: {
    ...
    extra_firmware+: [{
      ...
      source: { name: 'rkwifibt', subpath: 'firmware/broadcom/AP6275_PCIE' },
      files: [
        // files 元素 object 形态：src 含 wifi/ 或 bt/ 前缀（rkwifibt
        // 仓库内子目录），dest 是相对 ``dest`` 字段的扁平文件名。
        { src: 'wifi/fw_bcm43752a2_pcie_ag.bin', dest: 'fw_bcm43752a2_pcie_ag.bin' },
        { src: 'wifi/clm_bcm43752a2_pcie_ag.blob', dest: 'clm_bcm43752a2_pcie_ag.blob' },
        { src: 'wifi/nvram_ap6275p.txt', dest: 'nvram_ap6275p.txt' },
