// Orange Pi CM5 Tablet 板级配置：声明设备树、产品变体和板载硬件策略。
...
// 承担两项额外职责：
//
// 1. RK3588S SoC 通路（``components/platform/rockchip/rk3588s/config.jsonnet``）首次原生
...
// cm5-tablet 上 NPU 状态未验，apply 阶段 dmesg 判断后视情起独立 change）。
{
  ...
  kernel+: {
    ...
    // 状态使首版自然屏蔽）。
    device_tree+: { name: 'rk3588s-orangepi-cm5-tablet' },
  },
  rootfs+: {
    extra_firmware+: [{
      name: 'radxa',
