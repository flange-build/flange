// Radxa Cubie A7A 板级配置：声明设备树、产品条件和板载硬件策略。
...

{
  ...
  kernel+: {
    device_tree+: { name: 'sun60i-a733-cubie-a7a' },
  },
  kernel_device+: {
  ...
  boot+: {
    overlays+: {
      ...
      enabled: ['cubie-a7a-enable-sunxi-ac101-sound-card.dtbo']
        + (if product == 'meizu-e3-bringup' then [
          // meizu-e3-panel 的 panel overlay 由 packages 机制注入
          // boot.overlays.package（仅 meizu-e3-bringup product 启用包时注入）。
