// Rockchip RK3506B SoC 层配置：声明芯片级源码、Kconfig、设备树和固件能力。
// RK3506B SoC 配置——ARM32、vendor FIT、TOS 与 CPU2 AMP。
{
  ...
  bootloader: {
    ...
    cross_compile: '/opt/arm-linux-gcc10/bin/arm-none-linux-gnueabihf-',
    // SoC 缺省配置供非 ATK board 使用；ATK board 按原厂 SDK 覆盖为
    // alientek_rk3506_defconfig + rk-amp.config。
    defconfig: ['rk3506_defconfig', 'rk3506b.config'],
    ...
    fit_pack: { external_data_offset: '0x1200', slot_size_kb: 2048, copies: 2 },
