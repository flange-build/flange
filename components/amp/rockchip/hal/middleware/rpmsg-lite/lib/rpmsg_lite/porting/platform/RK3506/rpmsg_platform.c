...
static void rpmsg_master_cb(struct MBOX_CMD_DAT *msg, void *args)
{
    ...
    if (rx_msg.DATA != RL_RPMSG_MAGIC) {
        HAL_DBG("rpmsg master: mailbox data error!\n");
    }
...
static struct MBOX_CLIENT mbox_clr[MBOX_CNT * MBOX_CHAN_CNT] =
{
    ...
    { "mbox2-clr2", RL_PLATFORM_R_IRQ(3), rpmsg_remote_cb, (void *)MBOX_CH_0 },
};
#endif
        ...
        if (register_count % 2 == 0) {
            if (cpu_id == RL_GET_M_CPU_ID(vector_id)) {
                if (register_count == 0) {
                    ...
                    if (ret) {
                        HAL_DBG("mbox master client register failed, ret=%d\n", ret);
                    }
                }
                ...
                HAL_IRQ_HANDLER_SetIRQHandler(mbox_cl[RL_GET_R_CPU_ID(vector_id)]->irq, rpmsg_mbox_isr, rl_pMBox[RL_GET_R_CPU_ID(vector_id)]);
            } else {
                ...
                HAL_IRQ_HANDLER_SetIRQHandler(mbox_cl[cpu_id]->irq, rpmsg_mbox_isr, rl_pMBox[cpu_id]);
            }
        }
        register_count++;
#else
...
int32_t platform_interrupt_enable(uint32_t vector_id)
{
    ...
    if (disable_counter < 2 * RL_MAX_INSTANCE_NUM) {
        ...
        if (cpu_id == RL_GET_M_CPU_ID(vector_id)) {
            mbox_cl[RL_GET_R_CPU_ID(vector_id)] = &mbox_clm[RL_GET_R_CPU_ID(vector_id)];
            HAL_GIC_Enable(mbox_cl[RL_GET_R_CPU_ID(vector_id)]->irq);
        } else {
            mbox_cl[cpu_id] = &mbox_clr[cpu_id];
            HAL_GIC_Enable(mbox_cl[cpu_id]->irq);
        }
#else
        mbox_cl[cpu_id] = &mbox_clr[cpu_id];
        HAL_INTMUX_EnableIRQ(mbox_cl[cpu_id]->irq);
#endif
#endif /* RL_PLATFORM_USING_MBOX */
    }
...
int32_t platform_interrupt_disable(uint32_t vector_id)
{
    ...
    if (disable_counter < 2 * RL_MAX_INSTANCE_NUM) {
        ...
        if (cpu_id == RL_GET_M_CPU_ID(vector_id)) {
            mbox_cl[RL_GET_R_CPU_ID(vector_id)] = &mbox_clm[RL_GET_R_CPU_ID(vector_id)];
            HAL_GIC_Disable(mbox_cl[RL_GET_R_CPU_ID(vector_id)]->irq);
        } else {
            mbox_cl[cpu_id] = &mbox_clr[cpu_id];
            HAL_GIC_Disable(mbox_cl[cpu_id]->irq);
        }
#else
        mbox_cl[cpu_id] = &mbox_clr[cpu_id];
        HAL_INTMUX_DisableIRQ(mbox_cl[cpu_id]->irq);
#endif
#endif /* RL_PLATFORM_USING_MBOX */
    }
    disable_counter++;
