...
static int32_t first_notify, first_register = 0;
...
int32_t platform_init_interrupt(uint32_t vector_id, void *isr_data)
{
    ...
    if (isr_counter < 2 * RL_MAX_INSTANCE_NUM) {
        ...
        if (cpu_id == RL_GET_M_CPU_ID(vector_id)) {
            HAL_GIC_SetIRouter(rl_mbox_m_irq(cpu_id), CPU_GET_AFFINITY(cpu_id, 0));
            HAL_IRQ_HANDLER_SetIRQHandler(rl_mbox_m_irq(cpu_id), rpmsg_mbox_isr, rl_pMBox[cpu_id]);
        } else {
            HAL_GIC_SetIRouter(rl_mbox_r_irq(cpu_id), CPU_GET_AFFINITY(cpu_id, 0));
            HAL_IRQ_HANDLER_SetIRQHandler(rl_mbox_r_irq(cpu_id), rpmsg_mbox_isr, rl_pMBox[cpu_id]);
        }
        ...
        register_count++;
#else
        HAL_INTMUX_SetIRQHandler(MBOX_BB3_IRQn, rpmsg_mbox_isr, rl_pMBox[3]);
        if (register_count % 2 == 0) {
...
#ifdef HAL_AP_CORE
        if (cpu_id == RL_GET_M_CPU_ID(vector_id)) {
            HAL_GIC_Enable(rl_mbox_m_irq(cpu_id));
        } else {
            HAL_GIC_Enable(rl_mbox_r_irq(cpu_id));
        }
#else
        HAL_INTMUX_EnableIRQ(MBOX_BB3_IRQn);
#endif
#endif /* RL_PLATFORM_USING_MBOX */
    }
...
#ifdef HAL_AP_CORE
        if (cpu_id == RL_GET_M_CPU_ID(vector_id)) {
            HAL_GIC_Disable(rl_mbox_m_irq(cpu_id));
        } else {
            HAL_GIC_Disable(rl_mbox_r_irq(cpu_id));
        }
#else
        HAL_INTMUX_DisableIRQ(MBOX_BB3_IRQn);
#endif
#endif /* RL_PLATFORM_USING_MBOX */
    }
    disable_counter++;
