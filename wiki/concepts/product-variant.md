...
`lunch <board>-<product>-<variant>` 选择构建目标，持久化到 `.flange/current_config`，后续 `flange build/flash` 命令均以此为准。

## 关键设计要点

- **product**：对应同一硬件板的不同软件套件或使用场景，如 `default`、`smart-display`；在 board `config.py` 的 `products` 列表中声明
- **variant**：通常为 `release` / `debug`；debug 通过 `+packages:debug` 追加调试工具；在 `variants` 列表中声明
