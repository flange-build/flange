...
刷写走 **edl-ng**（与 Q6A 同一份，随仓 tools/，EDL 9008），EDL 经 **JCTL 跳线**进入。内核走
...
| boot 组件 | `grub-mkimage` BOOTAA64.EFI + ESP(FAT) | `builder/extlinux.py` extlinux.conf + boot.img(**FAT** efi) |
...
| 刷写工具 | edl-ng（write-sector 整盘） | **edl-ng（同一份）按分区 rawprogram** |
| flash 命令 | `edl-ng --memory UFS write-sector 0` | `edl-ng --memory emmc rawprogram <rawprogram>` |
...
edl-ng rawprogram **按分区**把 boot/rootfs 写进既有槽位（label efi/rootfs），不碰 vendor 槽。
...
boot.img（**FAT** ESP，匹配 vendor `disk-sdcard.img.esp`）。⚠️ Armbian 用定制 `boot-qrb2210.cmd`（显式 load 地址规避 ABL
...
通用 defconfig（含 qcom）——实测 v7.0 .config 已 builtin eMMC(SDHCI_MSM)/EXT4/VFAT/
SERIAL_QCOM_GENI/FW_LOADER_COMPRESS_ZSTD，DRM_MSM/ATH10K=m，故 enable_configs 留空。

## 刷写（QualcommQrb2210FlashStrategy）

- 系统（每次）：`edl-ng --loader <firehose> --memory emmc rawprogram flange_rawprogram.xml`
  （刷写前临时目录建 boot.img/rootfs.img 符号链接）—— flange rawprogram（`image.py`
  生成）仅含 boot(label efi)/rootfs 两条目。
- vendor 固件（bring-up 一次）：`flange flash --spi-firmware` → `edl-ng --loader <firehose>
  --memory emmc rawprogram rawprogram*.xml patch*.xml`（vendor 预编包）。
