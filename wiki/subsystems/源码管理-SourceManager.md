...
- **Shallow clone**：`_clone` 默认 `--depth 1` 减小克隆体积；若 config 指定 `commit` 则追加 `--no-single-branch` 后 `_fetch_checkout` 精确切到该 commit
- **增量更新**：`_ensure_repo` 调用 `_rev_parse` 取当前 HEAD；与 config 中期望 commit 比对，不同才 `_fetch_reset_branch` 或 `_fetch_checkout`
