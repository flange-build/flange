...
## [2026-06-21] sync | 新增 qualcommqrb2210 平台 + arduino-uno-q 板（Arduino UNO Q）

flange 第二个 Qualcomm 平台落地（openspec change `add-qrb2210-arduino-uno-q`）。QRB2210/QCM2290（Dragonwing，代号 Imola）与 [[qualcommqcs6490 平台]]（UEFI/GRUB）互补，落在 **U-Boot extlinux 世界**：`ABL→U-Boot(Android boot.img)→sysboot/extlinux→Linux`，`boot.py` 复用 `builder/extlinux.py`。**关键架构认知**：整盘 vs 按分区的分水岭不是「Q6A 整盘、UNO Q 分区」表面对比，而是 **bootloader 与 OS 是否共享同一块介质**——Q6A bootloader 在独立 SPI NOR、OS 在独立 UFS 故能 `write-sector 0` 重建 GPT；UNO Q 的 XBL/ABL/TZ/HYP/U-Boot 与 boot/rootfs **挤在同一块 eMMC 固定 vendor GPT（约 67 分区）**，只能用 `edl-ng ... rawprogram` 按分区写既有 boot/rootfs 槽位。内核 **mainline Linux v7.0**（`qrb2210-arduino-imola.dts` 已上游）、GPU Adreno 702 freedreno、Wi-Fi mainline ath10k、刷写复用 Q6A 的 `edl-ng`（按分区 rawprogram，EDL 经 JCTL 跳线）。新增平台 config/SoC config/board config + `builder/platforms/qualcommqrb2210/`（6 规则 + __init__）+ `QualcommQrb2210FlashStrategy`。已 Python 级验证（registry 发现、lunch target、builder 实例化、flash 注册、rawprogram XML）；Docker 编译 / vendor 固件包 / 实板启动待 bring-up。新增 [[qualcommqrb2210 平台]] + [[arduino-uno-q]]。

## [2026-06-21] sync | arduino-uno-q flange build 真实验证 + 刷写改 edl-ng

承上一条。本会话用 Docker 真实 `flange build` 逐组件验证 qualcommqrb2210：**kernel** 全量构建通过（clone mainline v7.0 + arm64 defconfig + 编译 → Image 50MB + `qrb2210-arduino-imola.dtb` + modules，~44min）；**boot** 产出 128MiB FAT32（label efi）含 extlinux.conf/Image/dtb，实测 `extlinux.conf` 的 devicetree/root=PARTLABEL=rootfs/console=ttyMSM0 正确；**image** rawprogram 对齐 vendor 布局；**flash-config** 经 FlashConfigGenerator 正确（flash_tool=edl-ng）。**rootfs** 的 chroot+qemu binfmt+base apt+key/源机制均通过，仅 qcom-ppa HTTPS 被 sandbox egress 代理 MITM 阻塞（环境限制，非代码问题）。**关键实证修正**：① defconfig dump 证实 arm64 defconfig 已 builtin 全部 root-critical 项（MMC_SDHCI_MSM/EXT4/VFAT/SERIAL_QCOM_GENI 等），原 enable_configs 用错名 SERIAL_MSM_GENI → 清空；② armbian/qcombin rawprogram0.xml 实证 boot→label `efi`@985408(512MiB FAT ESP)、rootfs→`rootfs`@2033984，boot.py 改产 FAT 对齐；③ dts 实证 a702_zap.mbn(Adreno 702)/ath10k/agatti.dtsi/console。**刷写按反馈改用 Q6A 同款 edl-ng**（继承 QualcommFlashStrategy，按分区 rawprogram，非整盘 write-sector），不引入 qdl。波及 [[qualcommqrb2210 平台]]、[[arduino-uno-q]]。

## [2026-06-21] sync | arduino-uno-q rootfs + 完整 flange build 端到端验证通过

承上。绕过 sandbox egress 代理 MITM（构建容器内注册 qemu-aarch64 binfmt + 临时把宿主 egress CA 注入 chroot 信任库让 qcom-ppa HTTPS 通过；临时 rootfs.py 改动已 git checkout 还原，未提交 sandbox 专属代码）后，**rootfs 完整构建通过**（3GB ext4，exit 0）：实测产物含 `linux-firmware-dragonwing`(install ok)、`/lib/firmware/qcom/qcm2290/a702_zap.mbn.zst`（Adreno 702 zap，dts 引用项，解开 §3.3/§8.4 firmware 落地疑问）、`/lib/firmware/ath10k/` 树、Mesa `msm_dri.so`+`libvulkan_freedreno.so`。随后**完整 `flange build image` 端到端通过**：kernel/bootloader/rootfs/boot 缓存命中 → image(rawprogram) + flash-config.json 生成，全链 exit 0；产物齐（kernel Image/dtb + boot.img(128MB FAT ESP) + rootfs.img(3G) + flange_rawprogram.xml + flash-config.json(flash_tool=edl-ng, boot fat32@0xF0940 / rootfs ext4@0x1F0940)）。至此 qualcommqrb2210 全组件（含 rootfs）均经真实 Docker 构建验证；仅余实板 bring-up（U-Boot load 地址 / EDL 刷写 / 启动全栈）待硬件。波及 [[qualcommqrb2210 平台]]、[[arduino-uno-q]]。
