...
构建期由 `builder.firmware_panel` 编为 mainline 兼容的 panel.bin（16B header = `"MIPI DBI"` magic 8B + 7B `0x00` reserved + 1B `file_format_version = 0x01`；其后是 cmd 序列与 NOP-encoded delay），落 rootfs `/lib/firmware/panel-mipi-dbi-spi.bin`。Driver 不读 DT `firmware-name` 属性，固定按 `<compatible[0]>.bin` 在 `/lib/firmware/` 下查找——dest 名称固定不可改。
