...
**面板型号在阶段 1 用实机 I2C 扫描确定**（探到 0x55 ⇒ ST7123；探到 0x14 ⇒ GT911 + ILI9881C），
**只实现实机上的那一种**；代码里保留一处显式探测，探到另一型号时明确报错说明未支持。
不预先实现双份 init 表 —— 两份 init 命令数组合计近 12 KB 源码，且无从验证。
...
| PPA 缩放+旋转的脏矩形坐标变换写错 | 画面错位/撕裂 | 阶段 1 单独用固定图案验证四个角与非对称矩形 |
| 面板批次与手上实机不符 | init 序列不匹配、黑屏 | 显式 I2C 探测 + 探到未支持型号时明确报错 |
| TinyUSB UVC device 不可用 | 摄像头功能落空 | 已排最后阶段，明确允许砍 |
| 全局内核 config 触发全板重建 | 一次性长构建 | 排在最后阶段一次性完成 |
| 640×360 放大后文字发虚 | 观感下降 | 2× 整数放大无插值；若仍不可接受，回退选项是加声明 720p 模式由 host 选（代价是帧率） |
