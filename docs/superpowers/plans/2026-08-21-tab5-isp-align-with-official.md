...
## 0. 实施状态（2026-08-23 回填）

> ⛔ **T0–T13 的代码全部实现完毕并已提交（14 个 commit），但一次都没有烧过板。**
> 已验证的只有宿主机侧：9 组共 **3258** 个用例、两档 clean build 零告警、
> USB 描述符两档与 `8d9e68c3` 逐字节一致、`isp_cal_extract.py --check` 过、
> `managed_components/` 仍是 12 个目录。
>
> **每个任务里那些「烧板 / 实拍 / 判据」类的步骤一律没有执行。**
> 因此只有 **T0 / T1（两个纯宿主机任务，标题里就写着「不上板」）与 T14 的四个 Step**
> 勾了复选框；**T2–T13 的复选框故意保持未勾选** —— 它们把「写代码」与「上板验证」
> 混在同一个列表里，勾上会把没做过的事标成做过了。
> 这些任务的实际进度以任务标题下的「实施状态」横幅为准。
>
> **上板怎么验**：`components/packages/tab5-all-in-one/firmware/README.md` 的
> 「**上板验证清单**」把 T2–T13 的全部判据整合成一份**从底层往上**的有序清单
> （阶段 0 烧板前闸门 → 1 链路基本面 → 2 统计抽头在不在画面上 → 3 控制律收敛 →
> 4 黑电平判定点 → 5 观感 → 6 复合回归），并标出三个 ⭐ 判定点与「关掉哪个退回哪个 commit」。

| 任务 | commit | 代码 | 上板 |
|---|---|---|---|
| T0 标定提取 | `543578b4` | ✅ | 不适用 |
| T1 纯逻辑映射层 | `9883348f` | ✅ | 不适用 |
| （计划外提前）gamma + 逆 gamma + AE 目标 62 | `751b24bb` | ✅ | ⏳ |
| T2 BF + Demosaic | `d8ce8ca0` | ✅ | ⏳ |
| T3 LSC | `e412fcae` | ✅ | ⏳ |
| T4 SHARP + Color | `cb8aa3f5` | ✅ | ⏳ |
| T5 硬件 AE 统计（观测） | `93f95c6c` | ✅ | ⏳ ⭐ρ |
| T6 AE 切硬件源 | `b8092878` | ✅ | ⏳ |
| T7 黑电平量具 | `8d9e68c3` | ✅ | ⏳ ⭐基座 |
| T8 硬件 AWB 统计（观测） | `7679189a` | ✅ | ⏳ |
| T9 AWB 切硬件源 | `7f2c4f26` | ✅ | ⏳ **R1** |
| T10 官方 19 档 CCM | `cb988504` | ✅ | ⏳ |
| T11 直方图 + env.luma | `477e8915` | ✅ | ⏳ ⭐env |
| T12 gamma 动态选档 + 背光 | `2b7f7d3b` | ✅ | ⏳ |
| T13 LSC/饱和度按 CCT | `e6201fc3` | ✅ | ⏳ |
| T14 复合回归 + 文档收口 | 本轮 | ✅ 文档 | ⏳ 回归 |

### 0.1 与计划不符之处（实施时的有意偏离，全部已在 commit message 里交代）

| # | 计划怎么写 | 实现怎么做 | 为什么 |
|---|---|---|---|
| 1 | **T6 Step 2**：换源必须按 ρ 缩放 AE 目标以保持观感不变（120 × 0.79 = 95） | **不缩放**，直接用官方的 62 | 那套算术的前提是「旧目标没有权威含义」。`751b24bb` 把目标改成官方的 62 之后前提就没了 —— **62 本来就是新口径下的权威值，再乘一次 ρ 等于把它搬回错误的域**。⇒ 计划里「画面观感与 T5 之前一致」那条判据**作废**，别拿它当回归判据 |
| 2 | **T7 Step 2** 的代码片段里 `CAM_SENSOR_BLC_ENABLE` 写的是 **1** | 默认 **0**（只读不写） | 那一行来自厂商寄存器表**不是数据手册**，`0x3902` 的位定义我们并不掌握；且计划自己的 Step 1 就是「先测再决定」。**第一次烧 = 写之前**，翻成 1 再烧 = 写之后，两次自检行一比就是 T7 要的前后差异。开着烧第一版的话，「基座本来是多少」这个数就永远拿不到了 |
| 3 | **T9 Step 3** 让 `cam_awb_step_hw()` 自己做 settle 分频，**Step 5** 又只在触发 oneshot 的那一拍调它 | 只由 oneshot 的触发节奏定周期，`cam_awb_step_hw()` 不碰 `st->settle` | **两处都分频，实际周期会变成 `INTERVAL² = 100` 拍 = 10 秒一次**，现场表现是「AWB 反应特别迟钝」。副作用：硬件路上 `reasons[]` 按**秒**累加（软件路按帧），`SKIP_PERIOD` 恒为 0 |
| 4 | **T12** 包含四件事（gamma 上线、AE 目标官方化、按 env 选档、背光切目标） | 前两件在 **`751b24bb`** 就提前做掉了（用户批准的计划外提前），T12 只做后两件 | 实拍画面几乎全黑 —— 我们直出线性光而主机按 sRGB 解码，等于又被压暗 50 倍。**gamma 不是画质微调，是这条链路上缺的一个必需环节**，不能排到最后 |
| 5 | **T13** 只在 AWB 判定 `APPLIED` 或 `SKIP_BAND` 的那一拍评估 LSC 档 | 改成在**输入端**判：`s_cct_k` 只在白点数达到官方 `min_counted` 时才更新 | 可信性是那个**数**的属性，不是某一拍的属性。于是 CCM 档 / LSC 档 / 饱和度三个消费者自动都只在可信 CCT 上动，与 AWB 拒绝动增益的门限口径一致 —— 更简单，且顺带管住了 CCM |
| 6 | **T13** 认为饱和度 130 对 AWB 有 ≈0.34% 的扰动 | **在默认路径上恰好为零** | T9 之后 AWB 采样点是 `BEFORE_CCM`，而 Color（对比度/饱和度）在 YUV 域、位于 CCM **下游** ⇒ 不在同一条路径上。硬件 AE 同理（`AFTER_DEMOSAIC`）。⚠️ 那 0.34% 并没有消失，它挪到了 `CAM_AWB_SOURCE = 0` / `CAM_AE_SOURCE = 0` 两个**回退档**上，以及自检行的「建议 R×/B×」上 |
| 7 | **T14 Step 1** 是「跑一遍复合回归并填表」 | 交付的是**可执行的规程**，实测列全部留空 | 前面十轮改动一次都没上过板 ⇒ 复合回归的前置条件（阶段 0–5）尚未满足。规程见 `firmware/README.md`「五项能力复合回归规程」 |
| 8 | **T14 Step 2.5**「把实测数字填进去」（ρ、黑电平基座、各档 CCM 强度、白点占比、`env.luma` 量程、fps、内存增量） | **一个都没填** | 同上。README 里凡是这类数字一律标 ⏳，并写明「上面是推算」 |
...
> **实施状态（2026-08-23，`543578b4`）**：✅ **全部完成**（纯宿主机任务，无上板步骤）
...
- [x] **Step 3：`test/isp_cal_extract.py`**
...
- [x] **Step 4：各段的提取规则（逐段写，每段一个 `emit_*` 函数）**
...
- [x] **Step 7：头文件的头部注释（照 `panel_init_data.h` / `tab5_kbd_map.h` 的规格）**
...
> **实施状态（2026-08-23，`9883348f`）**：✅ **全部完成**（纯宿主机任务，无上板步骤）；用例数已从 353 涨到 **532**（T2/T10/T11/T13 各有增补）
...
- [x] **Step 3：`cam_ccm_fold_wb()` 的实现（本计划最关键的 30 行）**
...
- [x] **Step 4：`test/test_cam_isp_map.c` 的用例分组（N ≥ 90）**
...
> **实施状态（2026-08-23，`d8ce8ca0`）**：🟡 **代码步骤已完成；上板步骤（Step 3 起的实拍判据）全部未执行**
...
> **实施状态（2026-08-23，`e412fcae`）**：🟡 **代码步骤已完成；上板步骤全部未执行**（含 Step 3 的芯片版本判定、Step 5 的转置开关判据）
...
> **实施状态（2026-08-23，`cb8aa3f5`）**：🟡 **代码步骤已完成；上板步骤全部未执行**
...
> **实施状态（2026-08-23，`93f95c6c`）**：🟡 **代码步骤已完成；上板步骤全部未执行** —— ⭐ **ρ ≈ 0.79 这个判定点还没有实测值**
...
> **实施状态（2026-08-23，`b8092878`）**：🟡 **代码步骤已完成；上板步骤全部未执行**。⚠️ Step 2 与实现相反，见下文「与计划不符」#1
...
> **实施状态（2026-08-23，`8d9e68c3`）**：🟡 **量具已交付；⭐ 黑电平基座实测值仍未取得**（这是 T10 强度参数的判定点）。⚠️ 开关默认值与计划的代码片段不同，见「与计划不符」#2
...
> **实施状态（2026-08-23，`7679189a`）**：🟡 **代码步骤已完成；上板步骤全部未执行**
...
> **实施状态（2026-08-23，`7f2c4f26`）**：🟡 **代码步骤已完成；上板步骤全部未执行** —— ⚠️ 这是**风险最高**的一步（R1），静置 5 分钟不发散的判据还没跑过。⚠️ Step 3/5 的分频有 bug，见「与计划不符」#3
...
> **实施状态（2026-08-23，`cb988504`）**：🟡 **代码步骤已完成；上板步骤全部未执行**。`CAM_CCM_STRENGTH_MAX` 停在**降级档 192**，等 T7 实测
...
> **实施状态（2026-08-23，`477e8915`）**：🟡 **代码步骤已完成；上板步骤全部未执行** —— ⭐ 「`env.luma` 四档里用得到几档」这个判定点还没有实测值
...
> **实施状态（2026-08-23，`2b7f7d3b`）**：🟡 **代码步骤已完成；上板步骤全部未执行**。⚠️ 其中两件（gamma 上线、AE 目标官方化）在 `751b24bb` 就已提前做掉，见「与计划不符」#4
...
> **实施状态（2026-08-23，`e6201fc3`）**：🟡 **代码步骤已完成；上板步骤全部未执行**。⚠️ 「CCT 可不可信」的判定方式与计划不同，见「与计划不符」#5
...
> **实施状态（2026-08-23，本轮）**：🟡 **文档收口已完成；复合回归本身未执行** —— Step 1 交付的是**可执行的规程**（`firmware/README.md` 的「五项能力复合回归规程」）而不是实测数字；Step 2/3/4 已完成，见下

目标：证明这一整轮改动**没有动到任何一项已实机验证的能力**，并把结论写进 README。

**Files:** Modify `README.md`；不改代码（除非回归发现问题）

- [x] **Step 1：复合回归清单（全部同时开着做）** —— ⚠️ **交付的是规程不是结论**，见 `firmware/README.md`「五项能力复合回归规程」。下表的判据已全部搬过去并补全了命令序列与「必须为 0 的计数」表；**实测列全空**
...
- [x] **Step 2：README 要新增/改写的段落** —— 已做，但第 5 条（「把实测数字填进去」）**做不了**：ρ / 黑电平基座 / 各档 CCM 强度 / 白点占比 / `env.luma` 量程 / fps / 内存增量**全部还没有实测值**，README 里一律标 ⏳
...
- [x] **Step 3：把"没做的"也写下来** —— 已做，见 `firmware/README.md` 的「本轮对齐之后**仍然与官方不同**的地方」（比下表多了 LSC 档间不插值、色调/亮度写 0、IPA 调度三行）
...
- [x] **Step 4：把基线文档的偏差补记** —— 已做：两份研究文档各追加一节 **§F 实施回填（2026-08-23）**，并在 `§C.10`（官方管线）与 `§A.3`（基线审计）就地加了 ⛔/⚠️ 指向该节（见下面「与基线文档不符/可补充之处」一节，逐条追加到两份研究文档的末尾，标注日期与实测来源）
