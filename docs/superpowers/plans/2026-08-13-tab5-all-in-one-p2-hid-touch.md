...
> ✅ **两半都完成了。** 宿主机回归测试全过（`test_touch_map.c`，32 用例）；
> **四角标定也已实机验证通过** —— 在 host 侧用 `evtest` 依次点四角，
> `ABS_MT_POSITION_X` 与 `ABS_MT_POSITION_Y` 各自都能跑到接近 0 与接近 32767，
> 反变换与轴向都对。
> 曾有一次抓取里所有触点的 Y 都落在满量程的 78%–99%，一度疑似 Y 轴映射问题；
> 四角标定通过 ⇒ 那就是当时手指位置所致，不是缺陷。
