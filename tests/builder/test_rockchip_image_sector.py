...
class TestResolveEntries512:
    ...
    def test_offsets_unchanged(self, builder):
        resolved = builder._layout(
            {"partitions": {"sector_size": 512, "entries": ENTRIES}})
        by_name = {e.name: e for e in resolved}
        assert by_name["idbloader"].offset_sectors == 0x40
        assert by_name["uboot"].offset_sectors == 0x4000
        assert by_name["boot"].offset_sectors == 0x8000
        assert by_name["idbloader"].size_sectors == 0x2000
...
class TestResolveEntries4096:
    ...
    def test_offsets_recomputed(self, builder):
        # 目标介质由 partitions.sector_size 声明 —— 生产就是这么表达的，
        # 不再从外部改构建器的私有状态。
        resolved = builder._layout(
            {"partitions": {"sector_size": 4096, "entries": ENTRIES}})
        by_name = {e.name: e for e in resolved}
        # 0x40 sector × 512 = 32768 B ÷ 4096 = 8
        assert by_name["idbloader"].offset_sectors == 8
        # 0x4000 sector × 512 = 8 MiB ÷ 4096 = 0x800
        assert by_name["uboot"].offset_sectors == 0x800
        # 0x8000 sector × 512 = 16 MiB ÷ 4096 = 0x1000
        assert by_name["boot"].offset_sectors == 0x1000
    ...
    def test_sizes_recomputed(self, builder):
        ...
        assert by_name["idbloader"].size_sectors == 0x400
        # 0x20000 sector × 512 = 64 MiB ÷ 4096 = 0x4000
        assert by_name["boot"].size_sectors == 0x4000
