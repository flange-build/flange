...
flange 平台 MAY 在不支持运行时 DT overlay 的启动链（典型如 grub-with-dtb + EDK2 UEFI）上启用「构建期 fdtoverlay 合并」，把 `kernel.device_tree.build_overlays` 声明的 overlay 与 base dtb 经 `fdtoverlay` 合并为单 dtb，覆盖式写入 rootfs `/boot/<dtb>.dtb`。该列表中的每一项 MUST 同时存在于 `boot.overlays` 四个来源数组之一。该能力 MUST 满足：

- **触发条件**：仅在 `kernel.device_tree.build_overlays` 非空时启用合并分支；为空时 MUST 走 base dtb 直拷路径，与未启用本能力前**字节等价**。
...
- **输入与查找**：合并 MUST 以 `target/device-tree-overlay/overlays/` 下的 `.dtbo` 文件为源（即既有 [[hardware-feature-packages]] `devicetree` component 的产物路径），按 `kernel.device_tree.build_overlays` 顺序传给 `fdtoverlay`。
...
- **THEN** rootfs `/boot/<dtb>.dtb` 是 `fdtoverlay` 合并 base dtb 与所有 `kernel.device_tree.build_overlays` 列出的 `.dtbo` 之后的产物
...
构建期合并能力 MUST 与 [[extlinux-dtb-overlays]] 定义的运行时 overlay 路径并存——`boot.overlays.vendor` / `boot.overlays.board` / `boot.overlays.intree` / `boot.overlays.package` 四源声明、basename 去重、`boot.overlays.enabled ⊆ 四源并集` 等既有约束 MUST 不被本能力修改。两种应用路径 MUST 由各平台 boot/rootfs 组件**择一**采用：U-Boot/extlinux 世界（rockchip/allwinner/amlogic）继续 fdtoverlays= 行运行时叠加；grub-with-dtb 世界（首例 qualcommqcs6490）采用本能力的构建期合并。
...
- **THEN** 关于 `boot.overlays.vendor` / `boot.overlays.board` / `boot.overlays.intree` / package overlay 四源的声明形式、basename 去重、`boot.overlays.enabled ⊆ 并集` 等约束 MUST 不被本变更修改
