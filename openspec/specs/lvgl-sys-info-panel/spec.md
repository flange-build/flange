# lvgl-sys-info-panel Specification

## Purpose

规范 LVGL + libdrm/GBM/EGL 系统信息面板 App（`lvgl_sys_info_drm`）的行为：以 LVGL 为 UI 框架，经 GBM + EGL + GLESv2 硬件 GL 管线直接呈现到 DRM 显示设备（无 compositor、独占屏幕），健壮地自动选择有屏的 DRM 节点与 connector，采集并可视化 CPU / 内存与存储 / 网络 / 系统与硬件四大类信息，提供一级/二级页面与触摸导航、设置项与持久化、开机自启机制，并约定打包与构建集成边界。首发在 A7A（Radxa Cubie A7A / Allwinner A733）+ 魅族 E3 屏实板验证，但设备选择、旋转、触摸逻辑保持通用，不写死板级常量，同时作为 flange "组件级 App" 能力的端到端示范。
