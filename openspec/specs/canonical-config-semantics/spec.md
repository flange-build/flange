...
最终 JSON MUST 经过完整 schema 与跨字段校验。未知字段、已知但没有任何生产消费者的字段、错误类型、无效 source 引用及互斥字段组合 MUST 使配置解析失败；测试或文档读取不得被视为生产消费者。

#### Scenario: 死参数被拒绝
- **WHEN** 配置声明一个仅在测试中断言、没有 builder 或 SourceManager 消费的字段
- **THEN** validator 失败并指出该字段没有 canonical 行为
