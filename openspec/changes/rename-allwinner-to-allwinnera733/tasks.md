...
- [ ] 3.1 修改 `platform/allwinnera733/config.py`：`PLATFORM.vendor` 从 `"allwinner"` 改为 `"allwinnera733"`
- [ ] 3.2 修改 `platform/allwinnera733/a733/config.py`：`SOC.platform` 从 `"allwinner"` 改为 `"allwinnera733"`
- [ ] 3.3 修改 `board/radxa-cubie-a7z/config.py`：`BOARD.platform` 从 `"allwinner"` 改为 `"allwinnera733"`

## 4. 构建器类名与 flash 注册键改写

- [ ] 4.1 重命名类：`AllwinnerKernelBuilder` → `AllwinnerA733KernelBuilder`（`builder/platforms/allwinnera733/kernel.py`）
- [ ] 4.2 重命名类：`AllwinnerBootloaderBuilder` → `AllwinnerA733BootloaderBuilder`（`bootloader.py`）
- [ ] 4.3 重命名类：`AllwinnerRootfsBuilder` → `AllwinnerA733RootfsBuilder`（`rootfs.py`）
- [ ] 4.4 重命名类：`AllwinnerBootBuilder` → `AllwinnerA733BootBuilder`（`boot.py`）
- [ ] 4.5 重命名类：`AllwinnerImageBuilder` → `AllwinnerA733ImageBuilder`（`image.py`）
- [ ] 4.6 更新 `builder/platforms/allwinnera733/__init__.py`：`create_builder` 中的 import 路径（`builder.platforms.allwinner.xxx` → `builder.platforms.allwinnera733.xxx`）和类名引用
- [ ] 4.7 修改 `builder/flash.py`：`AllwinnerFlashStrategy` 类名改为 `AllwinnerA733FlashStrategy`
- [ ] 4.8 修改 `builder/flash.py`：`_FLASH_STRATEGIES` 注册表键 `"allwinner"` 改为 `"allwinnera733"`

## 5. 代码内注释与日志字面量同步

- [ ] 5.1 `builder/platforms/allwinnera733/kernel.py` 中注释引用的 `platform/allwinner/a733/config.py` 改为 `platform/allwinnera733/a733/config.py`
- [ ] 5.2 其余代码文件内注释、docstring、错误信息里出现的"Allwinner 平台"/`allwinner` 字面量：若特指 flange 平台标识符，同步改为 `allwinnera733`；若指厂商/SoC 家族自然语义，保留原文
- [ ] 5.3 提交"rename: 配置、类名、注册键同步到 allwinnera733"
...
- [ ] 7.1 `ProjectSpec.md`：仅更新明确指向 flange 平台标识符的字面量（路径、代码引用），厂商名保留
- [ ] 7.2 `roadmap.md`：同上原则更新
- [ ] 7.3 `openspec/flange-build-tool-design.md`：同上原则更新
- [ ] 7.4 `CLAUDE.md`：检查无引用，无需改动（验证确认）
