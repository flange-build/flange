...
完整刷写在保留实板布局和 GUID 的同时，将官方模板中成对 A/B 分区的 GPT 第 54 字节
恢复为模板初值，清除历史不可启动/重试耗尽状态；不代写启动成功位。
该恢复仅用于完整计划，单分区刷写不写 GPT。否则沿用实板属性会使正常镜像继续被 ABL 拒绝启动。
...
首次容器导入在 systemd-timesyncd 恢复落盘时间后运行；实际需要导入时，使用单调时钟最多等待
本次 NTP 同步 30 秒，离线时继续使用本地归档。若 Docker 明确报告导入租约不存在，
仅重试一次以创建新事务；不重启 Docker 或清空已有镜像，其他错误和第二次失败仍向上传递。
每个归档导入前校验摘要，导入后校验平台与 config 身份，全部通过后才更新 Compose 并启动 App CLI。
...
用户报告新 U-Boot 烧入后系统无法启动；按用户要求，撤回本轮 board_fdt_blob_setup 早期捕获、
systemd 槽位解析及相关诊断改动，恢复到此前可启动的补丁版本。qbootctl 问题暂停处理，
不将其现有失败标为已解决。容器导入修复保留；重新构建和刷写由用户执行。

后续复核修正根因判断：回退后的 U-Boot 与此前正常镜像仅编译时间不同，机器码和设备树一致。
首刷新补丁前的 GPT 记录已显示 boot_a 的启动属性为 0x87（不可启动、重试耗尽），
后续全量刷写持续保留该值；不能据此断言新 U-Boot 导致此次故障。
恢复工作转为修复宿主全量刷写的 GPT 启动状态复原，继续暂停 qbootctl 运行时适配。
