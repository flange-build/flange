...
#### Scenario: 全量刷写恢复不可启动槽位
- **WHEN** 用户执行完整刷写，实板 GPT 留有已耗尽或不可启动的 A/B 槽位状态
- **THEN** 使用已校验的官方 GPT 模板恢复成对 A/B 分区的启动属性字节，并重新计算主备 GPT 的 CRC
- **AND** 保留实板 GUID、分区位置、其他属性及持久化分区，不将未启动系统标记为成功
- **AND** 单分区刷写不得执行此恢复或重写 GPT
