...
#### Scenario: 首次启动尚未校时或处于离线状态
- **WHEN** 需要从已锁定归档导入预装容器，而本次启动尚未完成时间同步
- **THEN** 在单调时钟限定的等待结束后继续本地导入，不以外网或 NTP 可用作为离线运行的必要条件

#### Scenario: 容器导入租约失效
- **WHEN** Docker 导入已校验的本地归档返回明确的租约不存在错误
- **THEN** 最多重新导入一次，并再次校验实际镜像平台与 config 摘要后才允许 App CLI 启动
- **AND** 其他错误、重复失败及身份不匹配仍使服务失败，保留具体诊断
