...
- 刷写用 **edl-ng**（与 Q6A 同一份，随仓 tools/）+ rawprogram/firehose，EDL 经 JCTL 跳线进入。
...
新增该平台既接入一块真实量产板，也为 flange 沉淀「高通 + U-Boot + extlinux + eMMC 固定 GPT + edl-ng 按分区 rawprogram」这一套与 Q6A 互补的高通形态。
...
- 刷写：`edl-ng --loader prog_firehose_ddr.elf --memory emmc rawprogram <rawprogram>.xml`（与 Q6A 同一份 edl-ng）；EDL VID:PID `05c6:9008`（与 Q6A 探测一致），JCTL 跳线进入。
...
- **新增刷写策略** `QualcommQrb2210FlashStrategy`（`builder/flash.py`，**继承** Q6A `QualcommFlashStrategy`）：基于 **edl-ng / EDL 模式**；`edl-ng --loader <firehose> --memory emmc rawprogram <flange rawprogram>.xml` **按分区**写 `boot`/`rootfs`（vendor 分区原样保留）；vendor bootloader blob 经 vendor rawprogram 单刷（bring-up 一次）；host 工具复用 Q6A 的 edl-ng。
...
- `qualcommqrb2210-flash`：Qualcomm edl-ng 刷写契约——`QualcommQrb2210FlashStrategy`（继承 Q6A）经 `edl-ng` 在 EDL 模式下 `--memory emmc rawprogram` **按分区**写 `boot`/`rootfs`，vendor bootloader blob 经 vendor rawprogram 单刷（bring-up）；设备探测（9008）、EDL（JCTL 跳线）诊断、edl-ng host 工具定位。
...
- **host 工具**：复用 Q6A 的 `edl-ng`（随仓 `tools/<os>/edl-ng/`，无新增依赖），EDL 模式经 JCTL 跳线进入。
