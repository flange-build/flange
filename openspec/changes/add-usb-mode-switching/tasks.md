...
- [ ] 4.1 平台探测：按序检查 mainline `usb_role` 节点与 Rockchip `otg_mode` 节点，探测表结构化便于扩展
- [ ] 4.2 角色查询，将各平台原始取值归一化为 host / device / none
- [ ] 4.3 角色切换原语：翻译为平台写入值、写入后回读校验、不符时报告期望值与实际值
- [ ] 4.4 不支持平台的显式报错路径，列出已尝试的节点路径，确保不出现静默失败
- [ ] 4.5 自查：确认切换原语中不含任何 gadget 启停逻辑
...
- [ ] 5.5 自锁保护：识别会切断当前控制通道的切换，超时回滚计时器与确认命令
- [ ] 5.6 持久化与复位：临时切换只改运行时状态，持久化切换跨重启保持，复位清除两者
- [ ] 5.7 开机自动进入场景：持久化场景优先于板级默认场景

## 6. 控制接口

- [ ] 6.1 unix socket 服务端与行分隔 JSON 协议，非法输入不致进程崩溃且不影响 USB 当前状态
- [ ] 6.2 `SO_PEERCRED` 分级授权：查询开放，变更要求 uid 0 或属于 `usbmode` group
- [ ] 6.3 `usb-mode` CLI：列举、查询、临时切换、持久化切换、复位、确认
- [ ] 6.4 CLI 的 adb 通道检测与自锁警告
