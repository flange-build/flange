...
### D9：控制协议为 JSON-RPC 2.0，行分隔传输

消息格式采用 JSON-RPC 2.0，传输仍为行分隔（每行一个 JSON 值），使 `socat` / `nc` 可直接手工调试。方法名用 `<域>.<动作>` 形式。

**为什么用标准协议而非自定义格式**：自定义的 `{"cmd":...}` / `{"ok":...}` 语义上已经是 RPC，但每个调用方都要自己约定错误表达、通知语义与批量语义。JSON-RPC 2.0 把这些定死了，且各语言都有现成客户端库 —— 本服务的调用方是本机业务程序，省掉它们各自造轮子是实打实的收益。

**领域错误映射到实现定义码区间**（规范保留的 `-32000`..`-32099`），而不是统统用 `-32603` Internal error：「场景不存在」是调用方该改的，「切换失败」可能要重试或查硬件，两者不该长一个样。调用方按码分支，不必解析消息文本。

**备选方案否决**：

- *gRPC* —— 否决。`grpcio` 在 arm64 嵌入式 rootfs 上是重依赖（要么本地编译、要么拉大包），而这是低频控制面操作，protobuf 的序列化收益为零，还会失去 `socat` 手工调试的能力。
- *保留自定义格式* —— 否决。见上，没有节省任何实现成本，却把错误与通知语义的约定成本转嫁给每个调用方。
