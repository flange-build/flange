...
- [x] 1.2 采集迁移前基线：在可实测的板子上记录默认 function 组合、枚举到的 VID/PID/产品名/序列号、`lsusb -v` 描述符输出，作为「行为等价」的判据
...
- [x] 6.1 unix socket 服务端与 JSON-RPC 2.0 协议（行分隔传输、通知、批量、标准与实现定义错误码），非法输入不致进程崩溃且不影响 USB 当前状态
...
> **当前状态：已在 Radxa ROCK 5B（172.17.10.177，经 SSH 带外通道）完成首轮实板验证。**
>
> 已验证：等价性（8.2）、场景切换与能力组合（8.4）、权限分级（8.6）、
> 自锁回滚（8.7）、开机路径（8.9）、基线采集（1.2）。
> 首轮验证发现并修复 6 个实板才能暴露的缺陷，见 field-findings.md。
>
> role 切换（8.5）已完成：给 Rockchip BSP 补 allow_userspace_control 内核
> patch 后，ROCK 5B 完成 device↔host 切换、编排顺序与自锁回滚的完整闭环验证。
>
> **本章已全部完成**（在 ROCK 5B 上）：13 项竞态逐项验证、adbd×systemd×FFS
> 专项、并发插拔、role 切换闭环。8.10 受限于手头只有一块板 —— 其余 13 块
> 板的配置已迁移并通过脚本比对等价，但无设备可验，保持「已迁移未验证」。
>
> 已有的替代覆盖：`tests/test_usbmoded.py` 用伪 configfs 验证了编排逻辑，
> 覆盖 13 项竞态中的 #2、#3、#4（部分）、#5、#6、#7、#8、#9、#12、#13。
> 未覆盖的 #1、#4（完整）、#10、#11 依赖真实硬件时序，只能实板验证。
> 自动化测试**不能**替代本章任何一项。
>
> 前置约束：任何一块板子的首次验证都必须在有串口可用的条件下进行 —— 本服务处于开机关键路径，失败会导致 adb 通道消失、设备完全失联。

- [x] 8.1 逐项验证 1.1 的 13 项竞态 checklist，每项独立确认，不以「USB 能用」整体代替
- [x] 8.2 行为等价验证：对照 1.2 基线，确认 VID/PID/产品名/序列号与 `lsusb -v` 描述符输出与迁移前一致
- [x] 8.3 adbd 交由 systemd 管理的专项验证：FunctionFS endpoint 文件描述符生命周期与 systemd 重启退避的配合，含反复启停不累积进程；此为本变更唯一的全新运行时行为，单独验收
- [x] 8.4 场景切换验证：能力集合变更、参数生效、未定义场景被拒、互斥组合被拒
- [x] 8.5 role 切换验证：支持平台上的 device↔host 切换与编排顺序、切到 host 后无残留 UDC 绑定；不支持平台上确认报错明确
- [x] 8.6 权限分级验证：普通用户可查询、切换被拒；`usbmode` group 成员可切换
- [x] 8.7 自锁回滚验证：经 adb 切换到自锁场景，超时后自动回滚且通道恢复；收到确认后场景保持
- [x] 8.8 并发验证：切换过程中触发 USB 插拔，确认操作被正确串行化且 gadget 状态不错乱
- [x] 8.9 开机路径验证：冷启动进入默认场景、持久化场景优先、启动失败有明确日志
- [x] 8.10 逐板验证：对每块可实测的板子跑完 8.1–8.2，记录结果（手头仅 Radxa ROCK 5B 一块，已完成；其余 13 块板无设备，保持「已迁移未验证」）

## 9. 文档

- [x] 9.1 更新 `components/app/adbd/README.md`：三层架构说明、能力清单与参数、场景定义格式、CLI 用法、socket 协议、持久化的失联风险
- [x] 9.2 在 `wiki/` 记录板级支持矩阵：各板已验证的能力组合、role 切换支持情况（已验证 / 已迁移未验证 / 不支持）
- [x] 9.3 在 `wiki/` 记录 13 项竞态知识及其对应的硬件现象，作为独立于代码注释的知识备份
