...
- [x] 9.3 反向通道经 link-id 命中 stock 驱动（**内核驱动零改动**）：app `main.c` 的 `RL_PLATFORM_SET_LINK_ID(M,R)` 由 (0,3)=0x03 改 **(1,0)=0x10** + board dts override `&rpmsg{rockchip,link-id=<0x10>}` 同步。link-id 低4位 R=0 使 rpmsg-lite `platform_notify`(`RL_GET_R_CPU_ID`) 把新消息发 mailbox **ch0** → 命中 stock `rk_rpmsg_rx_callback`→vq[0]（取 R=3 则发 ch3 落 tx 回调被丢）。**删除早期的 board kernel patch 0004**（曾给 `tx_callback` 补 `vring_interrupt(vq[0])` 迁就固件）——经核实 radxa linux-6.1-stan-rkr5.1 / rockchip-linux develop-5.10/6.1 三活跃分支均无此改，驱动保持 stock 与上游一致。接收仍 A2B ch3(222) 不变；`RL_ALLOW_CONSUMED_BUFFERS_NOTIFICATION=0` 故 tx 通道无流量、删 0004 安全。上板验证：UART4 `rpmsg: link up (link_id 0x10)`、`/sys/bus/rpmsg/devices` 出现 `rpmsg-ap3-ch0`、`/dev/rpmsg0` echo 通（走 stock ch0→rx_callback 路径）
