...
class FlashExecutor:
    ...
    def flash_all(self, no_wait: bool = False, no_reboot: bool = False, *, yes: bool = False):
        ...
        whole_disk_handled = self.strategy.flash_whole_disk(tool, self.target_dir, cfg)
        if whole_disk_handled is not True:
            for part in cfg.partitions:
                ...
                if not image.exists():
                    ...
                    continue
                if cfg.partition_format == "mtd" or cfg.storage_type == "spinand":
        ...
        print(_c(Role.SUCCESS, " ✓ 刷写完成") + _c(Role.MUTED, f"  {elapsed:.1f}s"))
        print(_c(Role.MUTED, sep))
        print()
    ...
    def flash_partition(self, name: str, no_wait: bool = False, no_reboot: bool = False, *, yes: bool = False):
        ...
        if not part:
            ...
            raise FlashError(f"未知分区: {requested_name}\n可用分区: {', '.join(available)}")
        image = self.target_dir / part.image
        if not image.exists():
        ...
        if name == "uboot" and self.config.storage_type == "spinand":
        ...
        if no_reboot:
            ...
            self.strategy.reboot(tool)
        print()
        print(_c(Role.SUCCESS, f" ✓ 分区 {requested_name} 刷写完成"))
    ...
    def flash_raw(self, device: str, *, yes: bool = False):
        ...
        if not yes:
            if os.environ.get("FLANGE_NO_INTERACTION") or not sys.stdin.isatty():
                raise FlashError("整盘刷写需要明确确认；核对 --raw 设备路径后添加 --yes")
            confirm = input(_c(Role.WARNING, "  ⚠ 将覆盖目标设备全部数据！确认？[y/N] "))
            if confirm.lower() != "y":
    ...
    def list_partitions(self):
        ...
        for p in cfg.partitions:
            exists = (_c(Role.SUCCESS, "✓") if (self.target_dir / p.image).exists()
                      else _c(Role.ERROR, "✗"))
...
def _cli_main(argv=None, *, public=False, target_dir=None, project_dir=None):
    ...
    parser = ArgumentParser(
        prog="flange flash" if public else "python3 -m builder.flash",
        description="flange 刷写工具",
    )
    subparsers = None if public else parser.add_subparsers(dest="command")

    # run 子命令
    run_parser = parser if public else subparsers.add_parser("run", help="执行刷写")
    if public:
        parser.set_defaults(command="run", target_dir=target_dir, project_dir=project_dir)
    else:
    ...
    run_parser.add_argument(
        ...
        nargs="?",
        const="lun0-only",
        choices=("lun0-only", "qcom"),
        metavar="PROFILE",
        help=("一次性初始化全新 Qualcomm UFS：lun0-only 为单用户 LUN；qcom 为官方 LUN 0-7 布局"),
    )
    ...
    if args.command == "run":
        ...
        if args.list_parts:
            ...
            if not hasattr(executor.strategy, "provision_ufs"):
                raise FlashError(f"平台 {executor.config.platform} 不支持 --provision-ufs")
            ...
            executor.strategy.provision_ufs(
                tool, executor.target_dir, device, profile=args.provision_ufs
            )
