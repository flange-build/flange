...
class RockchipRootfsBuilder(RootfsBuilder):
    ...
    def collect(self, src_dir: Path, config: dict) -> dict:
        outputs = super().collect(src_dir, config)
        if config.get("rootfs", {}).get("image_format", "ext4") == "ubi":
            outputs["ubi"] = outputs.pop("rootfs")
        return outputs
    ...
    def _build_ubi(self, rootfs_dir: Path, config: dict) -> None:
        ...
        min_io = (
            int(ubi["min_io_size"], 0)
            if isinstance(ubi["min_io_size"], str)
            else int(ubi["min_io_size"])
        )
        peb = int(ubi["peb_size"], 0) if isinstance(ubi["peb_size"], str) else int(ubi["peb_size"])
        subpage = (
            int(ubi["subpage_size"], 0)
            if isinstance(ubi["subpage_size"], str)
            else int(ubi["subpage_size"])
        )
        vid = (
            int(ubi["vid_hdr_offset"], 0)
            if isinstance(ubi["vid_hdr_offset"], str)
            else int(ubi["vid_hdr_offset"])
        )
        leb = int(ubi["leb_size"], 0) if isinstance(ubi["leb_size"], str) else int(ubi["leb_size"])
        max_leb = (
            int(ubi["max_leb_count"], 0)
            if isinstance(ubi["max_leb_count"], str)
            else int(ubi["max_leb_count"])
        )
        ...
        self._ensure_file_fits(ubifs, volume_size, "UBIFS", "rootfs.ubi.volume_size")
        ...
        physical_limit = self._ubi_physical_limit(
            config, peb, tool_root=self.context.tool_root if self.context else None
        )
        self._ensure_file_fits(self._output, physical_limit, "UBI", "rootfs MTD 可用容量")
    ...
    def _ensure_rootfs_fits_ubi(
        ...
        result = self.docker.run(["du", "-sb", str(rootfs_dir)], capture=True)
    ...
    def _ensure_file_fits(
        ...
        if size > limit:
            raise BuildError(
                f"{label} 产物 {size} bytes 超过 {limit_label} {limit} bytes；禁止截断写入。"
            )

    @staticmethod
    def _ubi_physical_limit(config: dict, peb_size: int, *, tool_root: Path | None = None) -> int:
        ...
        if partitions.get("format", "gpt") == "mtd":
            ...
            if not parameter.is_absolute():
                if tool_root is None:
                    raise ValueError("相对 parameter 路径需要显式 tool_root")
                parameter = tool_root / parameter
            ...
            rootfs_entry = next(
                entry for entry in partitions["entries"] if entry["name"] == "rootfs"
            )
        ...
        reserved = (
            int(reserved_value, 0) if isinstance(reserved_value, str) else int(reserved_value)
        )
