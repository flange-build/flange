...
class QualcommQrb2210FlashStrategy(QualcommFlashStrategy):
    """Qualcomm QRB2210 刷写策略 —— 与 Q6A 同款 **edl-ng / EDL**，但按分区 rawprogram。

    复用 Q6A QualcommFlashStrategy 的 edl-ng 工具定位 / reboot / pre_flash；区别在
    刷写形态：UNO Q 的 bootloader 与 OS 共享同一块 eMMC 的固定 vendor GPT（约 67
    分区），flange 只能**按分区**把 boot/rootfs 写进既有槽位，不整盘 write-sector、
    不重建 GPT。

    - 系统：``edl-ng --loader <firehose> --memory emmc rawprogram <flange rawprogram>.xml``
      —— flange rawprogram（image.py 生成）仅含 boot(label efi)/rootfs 两条目；
      刷写前在临时目录建 boot.img/rootfs.img 符号链接，使 edl-ng 按 basename 定位。
    - vendor bootloader（bring-up 一次）：``edl-ng --loader <firehose> --memory emmc
      rawprogram rawprogram0.xml patch0.xml`` 刷 XBL/ABL/TZ/HYP/U-Boot/GPT。
    - EDL 经 **JCTL 跳线** 进入；工具 edl-ng（随仓 tools/，与 Q6A 同一份）。
    """

    def find_tool(self, project_dir: Path) -> Path:
        """定位 edl-ng（随仓 ``tools/<os>/edl-ng/edl-ng`` 优先，PATH 兜底）。"""
        import shutil
        platform = "macos" if sys.platform == "darwin" else "linux"
        tool = project_dir / "tools" / platform / "edl-ng" / "edl-ng"
        if tool.exists():
        ...
        if exe:
            return Path(exe)
        raise FlashError(
            f"未找到 edl-ng。预期 {tool} 或 PATH（与 Q6A 同一份 edl-ng）。")

    def detect_device(self, tool: Path) -> Optional[DeviceInfo]:
        """复用 Q6A 的被动 USB 9008 探测，仅把 platform 标签换成本平台。"""
        info = super().detect_device(tool)
        if info:
            info.platform = "qualcommqrb2210"
        return info

    def wait_for_device(self, tool: Path, timeout: int = 30) -> DeviceInfo:
        """等待 EDL 设备；超时给 JCTL 跳线诊断。"""
        try:
            return FlashStrategy.wait_for_device(self, tool, timeout)
        except FlashError:
            raise FlashError(
    ...
    def _locate_loader(self, target_dir: Path) -> Path:
        """定位 firehose loader（bootloader.py 下载/自备的 EDL 固件包内）。"""
        ...
        if not matches:
            raise FlashError(
                ...
                "请确认 vendor 固件包（含 firehose loader）已放入该目录。")
        return matches[0]

    def flash_whole_disk(self, tool: Path, target_dir: Path,
                         config: "FlashConfig") -> bool:
        """按分区刷 boot/rootfs：edl-ng rawprogram + flange rawprogram。

        flange rawprogram 用 basename（boot.img/rootfs.img）引用镜像；这些镜像分散在
        target/boot/ 与 target/rootfs/，故先在临时目录建符号链接 + 放 rawprogram，
        让 edl-ng 在同目录下按 basename 解析（与 Q6A vendor 包"同目录"模型一致）。
        ...
        boot_img = target_dir / "boot" / "boot.img"
        rootfs_img = target_dir / "rootfs" / "rootfs.img"
        loader = self._locate_loader(target_dir)

        staging = Path(tempfile.mkdtemp(prefix="flange-edl-"))
        try:
            (staging / "flange_rawprogram.xml").write_text(rawprogram.read_text())
            for img in (boot_img, rootfs_img):
                if img.exists():
                    os.symlink(img, staging / img.name)
            _step(f"edl-ng rawprogram → eMMC（loader: {loader.name}）")
            cmd = [str(tool), "--loader", str(loader), "--memory", "emmc",
                   "rawprogram", "flange_rawprogram.xml"]
            if subprocess.run(cmd, cwd=str(staging)).returncode != 0:
                raise FlashError("edl-ng rawprogram 刷写失败")
        finally:
            import shutil
    ...
    def flash_spi_firmware(self, tool: Path, target_dir: Path,
        ...
        经 edl-ng rawprogram + vendor rawprogram*.xml + patch*.xml（armbian/qcombin
        预编包）。device 参数与其他策略签名对齐（edl-ng 不用它）。复用 CLI
        ``--spi-firmware`` 作为 vendor 固件 bring-up 入口（UNO Q 固件在 eMMC，非 SPI）。"""
        loader = self._locate_loader(target_dir)
        ...
        if not raws:
            raise FlashError(
                ...
                "请确认 vendor 固件包已放入该目录。")
        _step("edl-ng rawprogram → vendor bootloader 固件（eMMC，bring-up）")
        cmd = [str(tool), "--loader", str(loader), "--memory", "emmc",
               "rawprogram"]
        cmd += [p.name for p in raws]
        cmd += [p.name for p in patches]
        if subprocess.run(cmd, cwd=str(vendor_dir)).returncode != 0:
            raise FlashError("edl-ng vendor 固件刷写失败")
